...
export function checkRules({ rulesDir, roots, exclude = [], maxDepth = 6, extra = [] } = {}) {
  ...
  for (const r of rules) {
    ...
    for (const f of files) {
      ...
      // A DECLARED DEVIATION IS CHECKED FIRST, before the fingerprint gate.
      //
      // This ordering IS the fix, and the old order was the bug: `if (!re.test(raw))
      // continue` ran first, so a file was considered for override detection only if
      // it ALSO restated the rule. The cleanest files -- those that reference a rule
      // via `rule:<id>` instead of restating it, which is exactly what this directory
      // exists to encourage -- could declare an `overrides:` line that was never
      // surfaced. Found 2026-08-21 in Motherboard/CLAUDE.md, which had to be reworded
      // to restate part of the rule before its own declaration would appear.
      //
      // Silently dropping a declared deviation is the worst failure available here:
      // the point of the escape hatch is that divergence stays VISIBLE, and an
      // undetected override is indistinguishable from a file with no opinion.
      if (ov.test(raw)) {
        const line = raw.split(/\r?\n/).findIndex((l) => ov.test(l)) + 1;
        overrides.push({ rule: r.id, file: f, line });
        continue;
      }
      if (!re.test(raw)) continue; // no restatement here to report
      // A REFERENCE DOES NOT NEUTRALISE A RESTATEMENT — it only excuses it.
      //
      // The exemption is right in intent: a file pointing at the canonical rule
      // is doing the right thing, and a rule id sitting beside a hub-local fact
      // is not a copy. It was BLANKET, so a file that references a rule in one
      // line and keeps a stale forty-line copy in another was excused entirely
      // and never counted.
      //
      // That is the most likely shape, not a hypothetical: nobody deletes the
      // copy and adds the pointer in the same edit, so "pointer added, copy left
      // behind" is what a half-finished conversion looks like — and it is how
      // nine divergent copies of tool-priority came to make four mutually
      // exclusive claims.
      //
      // Measured 2026-08-24: `rules check` reported 0 restatements across 0
      // files while NINETEEN files restated a rule they also referenced.
      //
      // Still not a finding — flipping nineteen files to failures in one commit
      // is how a gate gets bypassed rather than fixed. Counted, so the unknown
      // stops reading as a clean result (rule:discernment-checks §2).
      if (raw.includes(`rule:${r.id}`)) {
        const line = raw.split(/\r?\n/).findIndex((l) => re.test(l)) + 1;
        referencedRestatements.push({ rule: r.id, file: f, line });
        continue;
      }
      const lines = raw.split(/\r?\n/);
      const hits = lines.map((l, i) => (re.test(l) ? i + 1 : 0)).filter(Boolean);
      // The file-level match tested the WHOLE text; this tests line by line. A
      // paraphrase spanning a line break therefore matches the file and no single
      // line, leaving `hits` empty — which rendered as `path/CLAUDE.md:` with
      // nothing after the colon, a file:line reference that goes nowhere. Found
      // 2026-09-16 on the first restatement the widened fingerprints ever caught
      // (rule:delegation-criteria in ManavDaehi/CLAUDE.md), because N76's whole
      // point is detectors that fire on prose rather than on copied lines — and
      // prose wraps. `spansLines` says so instead of rendering an empty number:
      // "matched, cannot point at one line" is not "matched at line ''".
      findings.push({ rule: r.id, file: f, hits, lines, spansLines: hits.length === 0 });
    }
  }
  ...
  return {
    ...
    findings,
    overrides,
    // Present and empty on a clean tree — never absent, so a caller reading
    // `.length` need not distinguish "none" from "this build does not report them".
    referencedRestatements,
    diagnostic: "ok",
...
export function selftest({ rulesDir } = {}) {
  ...
  // ── known-positive probes (N76 / issue #18) ───────────────────────────────
  //
  // The loop above proves a fingerprint matches its OWN BODY. That caught three
  // detectors whose YAML escaping made them incapable of firing, so it earns its
  // place — but it is not the check people read it as. A rule body is written in
  // that rule's house style, so matching it proves only that the fingerprint can
  // find a COPY of the rule.
  //
  // Measured 2026-09-16: 11 of 12 fingerprints do not fire on the same claim in
  // someone else's words, because every alternation in them is lifted verbatim
  // from the rule's own body. `rules check`'s "0 silent restatements" therefore
  // means "nobody copy-pasted a rule uncited" — not "nobody restated one".
  //
  // `rules/_probes.yml` carries, per rule, paraphrases that MUST fire and
  // near-miss prose that MUST NOT. Both lists are required: positives alone
  // invite widening a fingerprint until it matches everything, which is the
  // opposite error and is already recorded (nextjs-dev-server-port once flagged
  // 8 files to find 1).
  //
  // Absent or unparseable probes are reported, never silently skipped — a rule
  // with no probe is UNPROBED, which is a different fact from one that passes.
  try {
    // rulesDir is optional on this function and there is no module-level default
    // — loadRules(undefined) degrades to [] by design. Mirror that rather than
    // inventing a fallback path, and report the skip instead of swallowing it.
    const probesPath = rulesDir ? path.join(rulesDir, "_probes.yml") : null;
    if (probesPath && existsSync(probesPath)) {
      const probes = parseYaml(readFileSync(probesPath, "utf8")) ?? {};
      for (const r of loadRules(rulesDir)) {
        ...
        try {
          ...
          failures.push(`probe ${r.id}: fingerprint does not compile`);
          continue;
        }
        for (const sample of probe.positives ?? []) {
          const hit = re.test(sample);
          checks.push({ kind: "probe", id: r.id, want: true, pass: hit });
          if (!hit) failures.push(`probe ${r.id}: MUST fire on a real paraphrase but does not — "${String(sample).slice(0, 60)}…"`);
        }
        for (const sample of probe.negatives ?? []) {
          const hit = re.test(sample);
          checks.push({ kind: "probe", id: r.id, want: false, pass: !hit });
          if (hit) failures.push(`probe ${r.id}: MUST NOT fire on near-miss prose but does — "${String(sample).slice(0, 60)}…"`);
        }
      }
    } else {
      checks.push({ kind: "probe", id: "(file)", pass: true, absent: true, why: probesPath ? "no _probes.yml beside the rules" : "no rulesDir given, so probes were not looked for" });
    }
  } catch (err) {
    // A probe file that cannot be read is not zero probes. Say which.
    failures.push(`probes: could not be read — ${err.message}`);
  }
...
/**
 * Per-rule coverage: how many files the fingerprint MATCHES, and how many REFERENCE the
 * rule as `rule:<id>`.
 *
 * WHY (docs/ISSUES.md N35). `selftest` proves a fingerprint matches its own rule body,
 * which is written in that rule's own house style. It does not prove the fingerprint can
 * fire on how the claim is written in the WILD, and those differ in exactly the way that
 * matters: `never-commit-unless-asked` was `[Nn]ever commit unless` and matched ZERO
 * files across the tree while passing selftest, because the rule body bolds the whole
 * sentence and every real restatement bolds only `**Never commit**`.
 *
 * A rule at 0 matched / 0 referenced is NOT proof of a broken fingerprint — several
 * rules here are recent and may genuinely have nothing restating them. It is an
 * UNKNOWN. The entire point of reporting it is that an unknown currently renders
 * identically to a clean result, which is the failure mode this whole skill exists to
 * catch.
 *
 * @returns {Array<{id: string, matched: number, referenced: number, status: "firing"|"adopted"|"unexercised"}>}
 */
export function ruleCoverage({ rulesDir, roots, maxDepth = 6, extra = [] } = {}) {
  const rules = loadRules(rulesDir);
  const { files } = findCandidateFiles(roots, { maxDepth, extra });
  const raws = [];
  for (const f of files) {
    try {
      raws.push(readFileSync(f, "utf8"));
    } catch {
      /* unreadable file is not this report's problem */
    }
  }
  return rules
    .map((r) => {
      ...
      try {
        ...
        return { id: r.id, matched: 0, referenced: 0, status: "unexercised" };
      }
      const matched = raws.filter((raw) => re.test(raw)).length;
      const referenced = raws.filter((raw) => raw.includes(`rule:${r.id}`)).length;
      const status = matched > 0 ? "firing" : referenced > 0 ? "adopted" : "unexercised";
      return { id: r.id, matched, referenced, status };
    })
