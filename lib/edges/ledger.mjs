...
export async function readLedgerWithStats(jsonlPath) {
  if (!existsSync(jsonlPath)) return { rows: [], malformed: 0, unknownTypes: {}, manual: [] };
...
export async function renderMarkdown(jsonlPath, mdPath, opts = {}) {
  ...
  // The v1 launchd watcher was RETIRED 2026-08-14 (docs/DECISIONS.md). Drift is now
  // derived from content on demand by `reconcile`, so quiet does not mean "the watcher
  // died" — it means nothing changed, or nobody ran anything. This header advertised a
  // component that no longer exists for four days after its retirement, and `doctor`
  // could not see the contradiction because the text is generated, not written.
  // A stale entry is still worth flagging; it just is not evidence about a watcher.
  const tripwire = daysAgo === null
    ? "**Last entry: never** — nothing has been recorded yet. Run `/propagate status`."
    : daysAgo > 30
      ? `**⚠️ Last entry: ${daysAgo} days ago.** Run \`/propagate doctor\` — drift is derived on demand, so a long gap may just mean nothing has been run.`
      : daysAgo === 0
        ? "**Last entry: today.**"
      ...
      r.status === "done"
        ...
        : r.status === "wontfix"
          ...
          : r.status === "partial"
            ? "~ partial"
            : "open";
    return `| ${r.id} | ${date} | ${source} | ${change} | ${downstream} | ${statusBadge} |`;
  }
  ...
  if (rows.length === 0) {
    ...
    // Group under branch nodes — the reader `source_worktree` never had
    // (docs/DATA_MODEL.md §7: "3 rows written, NONE readers"). A row stamped
    // by `migrate-ledger --all-refs`/`--from-ref` carries
    // `source_worktree: {branch, commit}` naming the ref it was observed on;
    // everything else (the overwhelming majority of rows on disk today, and
    // every row from the plain working-tree path) has no stamp and renders
    // under one clearly-labelled default group rather than being silently
    // merged with a real branch's rows or dropped.
    const DEFAULT_GROUP = "  default"; // sentinel: git branch names cannot contain spaces
    const groups = new Map(); // group key -> rows[]
    for (const r of rows) {
      const branch = r.source_worktree && r.source_worktree.branch;
      const key = branch || DEFAULT_GROUP;
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(r);
    }
    const branchKeys = [...groups.keys()].filter((k) => k !== DEFAULT_GROUP).sort();
    const orderedKeys = groups.has(DEFAULT_GROUP) ? [DEFAULT_GROUP, ...branchKeys] : branchKeys;

    for (const key of orderedKeys) {
      const heading =
        key === DEFAULT_GROUP
          ? "## Default (no branch stamp)"
          : `## Branch: \`${key}\``;
      lines.push(heading, "", ...tableHeader);
      for (const r of groups.get(key)) lines.push(renderRow(r));
      lines.push("");
    }
  }
  lines.push("---");
  ...
  await writeFile(
    mdPath,
    `${body}\nGenerated by propagate at ${new Date().toISOString()}.\n`,
  );
  return true;
}
