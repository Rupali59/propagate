 ...
 */
/**
 * The ONE derivation of an edge id from a node id plus a downstream absolute
 * path (N40). Both halves are repo-relative node ids, so the same coupling
 * keeps one identity across checkouts and mounts.
 *
 * Exists because the derivation was previously open-coded at four sites — the
 * mint here, two matchers in cli.mjs, and (invisibly) inside two tests that
 * rebuilt it by hand. The tests kept passing while disagreeing with production,
 * which is why this is a function and not a convention.
 *
 * `edgeId` itself is frozen — see tests/watcher/edge-id-stability.test.mjs.
 * This changes what is fed to it, never how it hashes.
 */
export function edgeIdFor(nodeId, downstreamAbs, why) {
  return edgeId(nodeId, toNodeId(downstreamAbs), why);
}
...
/**
 * The ONE construction of a reconcile row (R1c).
 *
 * There were two independent row literals — the UNMATCHED branch and the
 * ordinary branch — each spelling out the same 14-key shape. A key added to
 * one and not the other is `undefined`, not an error: it renders as blank or
 * zero and no test fails. Same silent-accumulator shape as the doctor split
 * this plan's D4 addresses, one layer down.
 *
 * Every key is defaulted here, so a caller can only ever OMIT a field (getting
 * the explicit default) and never invent one that the other branch lacks.
 * `source`/`downstream` are merged rather than replaced so a partial side still
 * carries all four of its keys.
 *
 * `since` is DERIVED, never passed (R1d) — it was `last.ts` duplicated beside
 * its own source. It stays on the row because cli.mjs:4242 renders it as an
 * age, but there is now exactly one place that can compute it wrongly.
 *
 * @param {object} fields
 * @returns {object} a complete row
 */
function makeRow(fields) {
  const { source = {}, downstream = {}, ...rest } = fields;
  const row = {
    node_id: null,
    edge_id: null,
    state: null,
    ...
    unresolvable: null,
    ...rest,
    source: { path: null, ref: null, contentId: null, unresolvable: null, ...source },
    downstream: { path: null, ref: null, contentId: null, unresolvable: null, ...downstream },
  };
  row.since = row.last ? row.last.ts : null;
  return row;
}
...
export async function reconcile(workspaces, opts = {}) {
  ...
  for (const e of expanded) {
    ...
    if (e.unmatchedGlob) {
      ...
      rows.push(
        makeRow({
          node_id: nodeId,
          edge_id: edgeIdVal,
          source: { path: e.sourceAbs, ref: sourceRef },
          downstream: { ref: downstreamRef, unresolvable: "unmatched-glob" },
          state: "UNMATCHED",
          glob: e.glob,
          kind: e.kind,
          why: e.why,
    ...
    const edgeIdVal = edgeIdFor(nodeId, e.downstreamAbs, e.why);
    ...
    if (sRes.unresolvable || dRes.unresolvable) {
      // "a side absent" per the plan's own table: the normal, meaningful case
      // is the file genuinely not existing at this ref/working tree. Anything
      // else (no-repo, is-directory, lfs-pointer, read-error) is a DIFFERENT
      // kind of failure and must not be reported the same way.
      //
      // R1a: a not-found on ONE side must not mask a serious reason on the
      // OTHER. The old test asked "is either side not-found?" and answered
      // NOT_PRESENT_ON_REF, so an lfs-pointer source paired with a merely
      // absent downstream reported the benign state while this row's own
      // `unresolvable` said "lfs-pointer" — the row contradicted itself and
      // the benign half is the one a reader acts on. content-id.mjs:37-43 is
      // explicit that an LFS pointer "hashes stably while the real content
      // behind it changes freely", which is the case that must never read as
      // fine. So: a serious reason on EITHER side wins, and the reported
      // reason is always the one that produced the state.
      const serious =
        [sRes.unresolvable, dRes.unresolvable].find((r) => r && r !== "not-found") || null;
      unresolvableReason = serious || sRes.unresolvable || dRes.unresolvable;
      state = serious ? "UNRESOLVABLE" : "NOT_PRESENT_ON_REF";
      stats.unresolvable++;
    } else if (!last) {
