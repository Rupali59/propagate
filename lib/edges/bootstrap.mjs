 ...
 * @property {Array<object>} examinedAndDeferred
 */
 ...
 *
 * ISSUES.md N71/N39, GOTCHAS G61: a row can carry state === "NEVER_VERIFIED"
 * and STILL have been examined. `deferred` is a FLAG beside the state, not a
 * ninth state (lib/edges/migrate-ledger.mjs:507) — reconcile() attaches it
 * to an otherwise-NEVER_VERIFIED row whenever the edge was read, judged, and
 * deliberately deferred, with no later verifying event to clear it
 * (lib/edges/reconcile.mjs:283/398-419). Baselining that row would overwrite
 * a written human judgement with "asserted consistent now, without
 * inspection" — silently destroying it, under EITHER policy (co-commit
 * evidence does not make this safe: the deferral is a decision, not an
 * absence of evidence). So every deferred row is pulled out of the eligible
 * set BEFORE any policy classifies the rest, reported as its own outcome
 * rather than dropped (rule:discernment-checks §2 — "examined and parked" is
...
export function planBaseline(rows, policy, opts = {}) {
  ...
  // Split BEFORE any policy runs, so no policy branch below can ever see a
  // deferred row — this is the guard, not a filter one branch remembers to
  // apply.
  const eligible = [];
  for (const row of neverVerified) {
    if (row.deferred) {
      outcomes.examinedAndDeferred.push(row);
    } else {
      eligible.push(row);
    }
  }
