...
/**
 * Convenience wrapper combining `resolveObservedRef` (on `row.source`),
 * `resolveObservedPosition` (on `row.source.path`), and `resolveByKind` —
 * the full set of provenance fields one reconcile() row needs for a new
 * event payload. Each half is also exported individually above because the
 * RED tests exercise them independently (a failed ref lookup must not
 * depend on position resolution succeeding, and vice versa).
 *
 * `observed_on_ref` is always present (a string, or `null` on genuine
 * failure — never omitted, since `null` here is a meaningful fact, not an
 * absence). The two `*_error` fields are diagnostic-only and are omitted
 * from the returned object entirely when there is nothing to report, so a
 * normal successful event doesn't carry two permanent `null` columns.
 *
 * @param {{source?: {path?: string, ref?: string|null}}} row
...
export function resolveProvenance(row, byKindDefault) {
  const sourcePath = row && row.source && row.source.path;
  const refResult = resolveObservedRef(row && row.source);
  const posResult = resolveObservedPosition(sourcePath);
