...
/** True when an event carries any provenance-wedge position field at all. */
function hasRecordedPosition(event) {
  return event.observed_at_commit != null || event.observed_on_branch != null;
}

/** One event -> one rendered chain entry. */
function toEntry(event) {
  const position = hasRecordedPosition(event)
    ? {
        recorded: true,
        commit: event.observed_at_commit ?? null,
        branch: event.observed_on_branch ?? null,
        dirty: event.observed_dirty ?? null,
      }
    : {
        recorded: false,
        note: "position not recorded (written before provenance was tracked)",
      };
