...
function flattenAndDedupe(branchSets) {
  ...
  const duplicates = [];
  const processRows = [];
  // N41 — dedupeKey -> Map<ref, final status on that ref>.
  //
  // The dedupe below keeps whichever ref sorts first and drops the rest. That
  // is correct for the ROW (one logical row migrates once) and wrong for the
  // DISPOSITION: when two branches disagree about whether an edge is done, sort
  // order silently picks a winner and no one is told a human decision was
  // overridden. Recording every ref's final status here is what makes the
  // disagreement reportable at all.
  const finalStatusByRef = new Map();
  ...
  for (const { ref, head, rows } of branchSets) {
    ...
    for (const row of rows) {
      ...
      // The row's own status is the starting point; a status_change on this
      // same ref supersedes it below. `?? "open"` mirrors the reader's default.
      if (!finalStatusByRef.has(key)) finalStatusByRef.set(key, new Map());
      finalStatusByRef.get(key).set(ref, row.status ?? "open");
    ...
    // Fold this branch's transitions into its final status BEFORE the
    // duplicate filter below drops them. A duplicate branch's status_change
    // rows never reach `processRows` — which is exactly why its disposition
    // used to vanish without trace.
    const idToKey = new Map();
    for (const row of rows) if (ROW_OPENING_TYPES.has(row.type)) idToKey.set(row.id, dedupeKey(row));
    for (const row of rows) {
      if (row.type !== "status_change") continue;
      const key = idToKey.get(row.id);
      if (key && finalStatusByRef.has(key)) finalStatusByRef.get(key).set(ref, row.status ?? "open");
    }
  ...
  return { processRows, appearances, duplicates, finalStatusByRef };
}
...
export async function migrateLedger({ fromPath, intoPath, apply = false, allRefs = false, fromRef = null }) {
  ...
  const { processRows, appearances, duplicates, finalStatusByRef } = flattenAndDedupe(branchSets);

  // N41 — where the refs DISAGREE about a row's final status, that is a
  // contested human decision, not a dedupe detail. `contestedByKey` is
  // consulted when each row is written so the flag rides on the row itself, and
  // `contested` is returned so a caller can report it without re-deriving.
  //
  // A FLAG, NOT A NINTH STATE (docs/deferred/2026-08-20-two-tier-ref-aware-ledger.md
  // D5). The row keeps whatever content state the canonical ref gave it; the
  // disagreement is recorded alongside so a person reconciles it deliberately
  // rather than sort order doing it silently.
  const contestedByKey = new Map();
  for (const [key, byRef] of finalStatusByRef) {
    if (new Set(byRef.values()).size <= 1) continue; // agreement — nothing to report
    contestedByKey.set(key, [...byRef].map(([ref, status]) => ({ ref, status })));
  }
  ...
  for (const row of processRows) {
    ...
    if (ROW_OPENING_TYPES.has(row.type)) {
      ...
      const contestedHere = refMode ? contestedByKey.get(dedupeKey(row)) : undefined;
      if (contestedHere) withoutId.contested = contestedHere;
  ...
  return {
    ...
    duplicateEventsCollapsed: duplicates.length,
    duplicates,
    // Present and EMPTY when refMode ran and found agreement; empty in
    // plain mode too, where the question cannot arise. Never absent — a caller
    // checking `res.contested.length` must not have to distinguish "none" from
    // "this build does not report them" (rule:discernment-checks §2).
    contested: refMode
      ? [...contestedByKey].map(([key, dispositions]) => ({ key, dispositions }))
      : [],
  };
}
