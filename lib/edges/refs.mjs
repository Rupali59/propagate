 ...
 */
export async function enumerateRefs(repoRoot, { includeRemoteOnly = false } = {}) {
  ...
  try {
    ...
    const { stdout } = await runGit(repoRoot, [
      "for-each-ref",
      "--format=%(refname:short)%09%(objectname)%09%(upstream:track)%09%(committerdate:iso-strict)%09%(upstream:short)",
      "refs/heads/",
    ]);
    for (const line of stdout.split("\n")) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      // Split on tabs, not indexOf, now that there are five columns. Trailing
      ...
      refs.push({
        ...
        // `upstream:short` — the NAME of the tracking branch, which is a
        // different question from `upstream:track` (how far ahead/behind).
        // It was never asked for, so every local branch carried `upstream:
        // null` and read as "never pushed". Appended as the FIFTH column
        // rather than inserted, so the existing indices cannot shift.
        //
        // null, NOT "": here the two states are absent-vs-present, and there
        // is no third "asked and got an empty name" case — git yields "" only
        // when there is no upstream at all. Distinct from `upstreamTrack`,
        // whose "" genuinely means "asked, nothing to report".
        upstream: cols[4] ? cols[4] : null,
      });
    }
  } catch (err) {
    return { refs: [], error: errMessage(err) };
  ...
  // REMOTE-ONLY BRANCHES — opt-in, and the branch registry is what opts in.
  //
  // A branch pushed and then deleted locally, or never checked out on this
  // machine, exists nowhere in refs/heads. Measured on Vipin Kaushik
  // 2026-08-24: 24 local heads against the shell registry's 36. The missing 12
  // were all remote-only, and reading local heads alone made the registry
  // report every one as PRUNED — five classified as "recoverable" work that had
  // been lost. Nothing had been deleted.
  //
  // ref-resolver.sh had already fixed this and said why: "A branch nobody has
  // checked out locally is still work that exists; it is exactly what a
  // coordination tool must not miss." This carries that lesson rather than
  // rediscovering it a third time.
  //
  // OPT-IN because `bootstrap` and `migrate-ledger --all-refs` also call this
  // and read a ledger file per ref; widening their population silently would
  // change what those commands do.
  if (includeRemoteOnly) {
    const seen = new Set(refs.filter((r) => r.kind === "branch").map((r) => r.ref));
    try {
      const { stdout } = await runGit(repoRoot, [
        "for-each-ref",
      ...
      for (const line of stdout.split("\n")) {
        ...
        const [name, sha] = cols;
        if (!name || !sha) continue;
        if (name === "HEAD") continue; // origin/HEAD is a symref, not a branch
        if (seen.has(name)) continue; // a local branch already covers it
        refs.push({
          ref: name,
          kind: "branch",
          path: null,
          ...
          isCanonical: false,
          detached: false,
          // A remote ref has no LOCAL tracking state, so `upstream:track` is
          // INAPPLICABLE rather than unknown — "" by construction, exactly as
          // ref-resolver.sh records it. Inventing `[ahead 0]` would fabricate a
          // fact about a branch nobody tracks here.
          upstreamTrack: "",
          upstream: `origin/${name}`,
          lastCommitIso: cols[2] ?? "",
        });
      }
    } catch (err) {
      // A repo with no `origin` is normal — several in this tree have no remote.
      // Locals already enumerated stand; the absence is reported, not fatal.
      return { refs, error: null, remoteNote: errMessage(err) };
    }
  }
