...
import { INTEGRATIONS, STATE_DIR, SKILL_DIR } from "../core/config.mjs";
import { CLAUDE_SKILLS_REGISTRY_OFF } from "../core/paths.mjs";
...
// NULL-SAFE BY CONSTRUCTION, and this is not defensive padding.
//
// INTEGRATIONS.marketplaceDir is null on any machine that has not configured a
// marketplace — which, since the integration layer landed, is every machine except this
// author's. `path.join(null, …)` throws TypeError, and these are MODULE-LEVEL consts, so
// the throw happens at IMPORT time. lib/config.mjs's own header says a throw at module
// load "bricks watcher, CLI and UI simultaneously"; this is that hazard, one module over.
//
// Found by CI on Linux, not locally: two whole test FILES failed to load with
// `ERR_INVALID_ARG_TYPE: path must be of type string. Received null`, while the same
// suite passed 741/741 on the machine that has the directory. A skill going public cannot
// depend on its author's filesystem to import.
//
// null propagates instead of throwing: a consumer that needs the path checks for null and
// skips the feature, which is what the integration layer was for.
const under = (...parts) => (MARKETPLACE_DIR ? path.join(MARKETPLACE_DIR, ...parts) : null);

export const QUARANTINE_SKILLS = under("quarantine", "skills");
export const PROMOTED_SKILLS = under("tathya", "skills");
export const REAPED_ARCHIVE = under("quarantine", ".reaped");
...
export const DISARM_FILE = CLAUDE_SKILLS_REGISTRY_OFF;
