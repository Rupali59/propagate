    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        if (LEDGER_FILENAMES.has(e.name)) {
          ...
          // Current layout (docs/DECISIONS.md, "a propagation/ folder in every
          // workspace") — un-hidden legacy basename. `propagation/` is not a
          // dot-directory, so the walk already descends into it unfiltered;
          // this branch is the only piece that was missing, matching the
          // filename the same way the legacy `.propagation/` branch above does.
          ledgers.push({ path: full, kind: "propagation/ledger.jsonl" });
