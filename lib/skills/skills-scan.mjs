...
export const SKILLS_DIR = CLAUDE_SKILLS;
export const SKILL_LOCK = AGENTS_SKILL_LOCK;
export const CLAUDE_JSON = CLAUDE_USER_CONFIG;
export const PROJECTS_DIR = CLAUDE_PROJECTS;
...
export function readFrontmatterFields(skillMdPath) {
  ...
  let text;
  try {
    // READ THE WHOLE FILE, then bound the BLOCK — not the other way round.
    //
    // This used to `.slice(0, 8192)` before looking for the closing `---`, which
    // silently reported "no frontmatter" for any skill whose frontmatter ran past
    // 8 KB. Measured 2026-09-01: `ai-sdk` closes at byte 15705 and `nextjs` at
    // 14137 (Vercel ships large `metadata.docs:` blocks), so BOTH were counted as
    // having no name and no description — while both in fact carry a description
    // that is textbook WHEN-phrased ("Use when building AI-powered features …").
    //
    // The cost was not the two files. It was the CONCLUSION: doctor reported "2
    // with no name+description (cannot autotrigger)", which reads as the exact
    // failure `rule:description-standard` records costing three skills 3,755
    // sessions of disuse. A reader would have gone looking for a problem that does
    // not exist, and the skills that genuinely cannot autotrigger — if any ever
    // appear — were indistinguishable from these two.
    //
    // A SKILL.md is a document a person wrote; reading all of it costs nothing
    // worth protecting. The block is still bounded below, so a file with no
    // closing delimiter cannot make this scan the whole thing as frontmatter.
