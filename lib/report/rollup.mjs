...
import { discoverWorkspacesSync, nearestOwner, ownerCandidates } from "../core/discovery.mjs";
 ...
 */
// resolveOwnerRoot + buildOwnerCandidates MOVED 2026-09-23 to
// lib/core/discovery.mjs as `ownerCandidates` (N87 slice 1). They were private
// here while `doctor` answered the same conformance question from a DIFFERENT,
// marker-based population — and the two disagreed about exactly the workspaces
// that fail. One census, two readers; see that function header for the why.
...
export function rollup({ searchRoots, backlogResult, inventoryResult } = {}) {
  ...
  const owners = ownerCandidates(roots, discovery.workspaces);
...
export function renderRollup(result) {
  ...
  for (const o of result.perOwner) {
    ...
    if (o.layout) {
      if (o.layout.conforms) {
      ...
      } else if (o.layout.missing.length === 0 && o.layout.empty.length) {
        // N82: present and hollow. Saying "Missing 0 of 5" would be absurd, and
        // saying it MISSES `state/` would be false — the directory is there and
        // that is precisely why this read as migrated for so long.
        content.push(
          `**Layout** — NON-CONFORMANT, present but EMPTY: ` +
            `${o.layout.empty.map((m) => `\`${m}\``).join(", ")} ` +
            `${o.layout.empty.length === 1 ? "exists and holds" : "exist and hold"} no state. ` +
        ...
        content.push(
          `**Layout** — NON-CONFORMANT. Missing ${o.layout.missing.length} of ` +
            ...
            (o.layout.empty.length
              ? `; and ${o.layout.empty.map((m) => `\`${m}\``).join(", ")} present but empty`
              : "") +
            `. Required by \`docs/REFERENCE.md\` §"Propagation layout".`,
        );
      }
...
/**
 * sha256 (full 64-hex) of the bytes strictly between the opening body mark
 * and the start of the footer block. Returns `null` if either marker is
 * missing or out of order — a caller must not treat that as a zero-length
 * body.
 *
 * `bodyMark`/`footerMark` default to THIS module's own `ROLLUP_BODY_MARK`/
 * `ROLLUP_FOOTER_MARK`, so every existing zero-arg call site (this file,
 * `commands/rollup.mjs`, `tests/unit/rollup-render.test.mjs`) is unaffected.
 * They are parameters — not a second copy of this function — so a DIFFERENT
 * generated artifact (`lib/report/doc-reference.mjs`, `docs/REFERENCE.md`)
 * can reuse the same hash-footer mechanism under its own marks rather than
 * re-implementing it, which is the G20 failure this generalisation exists to
 * prevent: only the mark strings are new, the scanning/hashing logic is not
 * duplicated anywhere.
 */
export function bodyHash(text, bodyMark = ROLLUP_BODY_MARK, footerMark = ROLLUP_FOOTER_MARK) {
  const startIdx = text.indexOf(bodyMark);
  const footerIdx = text.indexOf(footerMark);
  if (startIdx === -1 || footerIdx === -1 || footerIdx <= startIdx) return null;
  const body = text.slice(startIdx + bodyMark.length, footerIdx);
 ...
 *
 * `footerMark` defaults to `ROLLUP_FOOTER_MARK`, same reasoning as
 * `bodyHash`'s default parameter above — every existing call site is
 * unaffected, and a second generated artifact can reuse this parser under
 * its own mark without a second implementation.
 */
export function parseFooter(text, footerMark = ROLLUP_FOOTER_MARK) {
  const idx = typeof text === "string" ? text.indexOf(footerMark) : -1;
  ...
  const block = text.slice(idx + footerMark.length, end);
