 ...
 *
 * Install and schedule integrity: plist, monitor, state.json. Misleading
 * rather than silent — a wrong answer here is visible to anyone who looks at
 * the named artifact. G-O is the exception that argues for vigilance, not
 * for S1: a destroyed plist read green for seven days, and the probe that
 * now catches it lives with the component, not here.
 *
 *
