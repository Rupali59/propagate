 ...
 *
 * Size caps and register hygiene. Real pressure, fully inspectable, and
 * nothing downstream is silently wrong while it is unaddressed.
 *
 *
...
export async function checkRegisters({ reporter, registersFn, gotchasSourcesFn, caps = {} }) {
  ...
  for (const row of overCap(result, caps)) {
    // Label is the KIND, detail is the instance (N87 slice 3). Built the other way
  // round, every row was its own kind and nothing could fold.
  reporter.warn("register over size cap", `${row.file} — ${row.lines} lines (cap ${row.cap})`);
  }
