 ...
 *
 * Owns the CENSUS and v3 conformance. A wrong population makes every other
 * number in the report meaningless, and it was silently wrong until
 * 2026-09-23 (N82 at S1, N87): `16/16 conform` over a set that excluded the
 * two failing workspaces, and `0/0 conform` rendered as a green tick.
 *
 *
...
import { isWorkspaceMarker, liveLedgerCandidates, sweepMarkers, ownerCandidates } from "../../core/discovery.mjs";
...
import { CLAUDE_RULES } from "../../core/paths.mjs";
...
try {
  const { conformanceReport, migrationVerdict } = await import("../../core/v3-layout.mjs");
  // N87 slice 1: the POPULATION, not the function, was the defect. Both this
  // and `rollup` already called `conformanceReport`; this side passed
  // `WORKSPACES`, which is MARKER-based — and a marker is something a workspace
  // only carries once it has adopted the layout, so the check for "did you adopt
  // the layout" was gated on having adopted it. propagate and Motion-Graphics
  // were not reported as failing; they were not reported at all.
  ...
  // AN EMPTY POPULATION IS NOT A PASS (N82, raised to S1 on this; GOTCHAS G68).
  // `offenders.length === 0` below is vacuously true over an empty set, so this
  // printed `✓ 0/0 conform` — a green tick for having examined nothing, and
  // indistinguishable from `✓ 22/22 conform` to anyone scanning the report.
  // GOALS.md goal 1 names `propagate doctor` as its derivation, so an
  // unmeasurable tree satisfied a goal by returning nothing. G24 is the same
  // hazard through the other door: a plausible wrong hub finds zero workspaces
  // and then reports healthy.
  //
  // Inconclusive rather than fail: nothing is BROKEN when there is nowhere to
  // look, and sending someone to fix working code is its own defect.
  if (rep.total === 0) {
    reporter.inconclusive(
      "workspaces conform to the v3 propagation layout",
      "the census is empty — 0 workspaces reachable, so nothing was examined. " +
        "Check `searchRoots`/`hubRoot` in the resolved config before reading this as healthy.",
    );
  } else {
  // N82: `rep.hollow` is a SEPARATE failure from `rep.offenders`, and folding it
  // in would print "X lacks " with nothing after "lacks" — every required path
  // is present, the state tree is empty. Two different repairs: an offender
  // needs files created, a hollow workspace needs its state written (or the
  // honest answer that it holds none, which is a decision about the tree).
  const failing = rep.offenders.length + rep.hollow.length;
  const parts = [];
  if (rep.offenders.length) {
    parts.push(
      `${rep.offenders.length} half-migrated — ` +
      rep.offenders.map((o) => `${o.name} lacks ${o.missing.join(", ")}`).join("; ") +
      `. A partial migration is the state that loses data.`,
    );
  }
  if (rep.hollow.length) {
    parts.push(
      `${rep.hollow.length} present but EMPTY — ` +
      rep.hollow.map((o) => `${o.name}'s ${o.empty.join(", ")} holds no state`).join("; ") +
      `. The directory exists, so this reads as migrated and is not.`,
    );
  }
  reporter.check(
    "workspaces conform to the v3 propagation layout",
    failing === 0,
    failing
      ? `${parts.join(" ")} See docs/REFERENCE.md §"Propagation layout".`
  ...
  // PR-001. The DECISION lives in `migrationVerdict` (lib/core/v3-layout.mjs),
  // pure and unit-tested, because as three inline branches here the only way to
  // exercise it was a multi-workspace fixture running the whole section — and
  // the coverage audit correctly reported the check as never having been seen
  // to fail.
  const verdict = migrationVerdict(rep);
  if (verdict.kind === "fail") {
    reporter.check(verdict.label, false, verdict.detail);
  } else if (verdict.kind === "pass") {
    reporter.check(verdict.label, true, verdict.detail);
  } else if (verdict.kind === "info") {
    reporter.info(verdict.label, verdict.detail);
  }
...
//
// NOT a reporter.check(), and the measurement is why: failing doctor for a
// description someone else wrote is a check the operator cannot act on, which is
// how a gate becomes noise.
//
// SPLIT BY OWNERSHIP, added 2026-09-01, because the combined number was
// unactionable. A skill under ~/.claude/skills may be a real directory (ours) or
// a SYMLINK into ~/.agents/skills or a plugin cache (third-party) -- and
// scanSkills reports the symlink path, so the two are indistinguishable without
// a realpath. Measured on the split: every skill lacking a description entirely
// was third-party, and none of ours was. That is the difference between "two
// skills cannot autotrigger, fix them" and "two skills someone else ships cannot
// autotrigger, and there is nothing you can do about it from here."
//
// DO NOT restate the counts in this comment. The version before this one said
// "Of 45 installed skills, 35 ... and 2 ..." and every one of those numbers was
// wrong within weeks (73 / 63 / 2 by 2026-09-01), sitting inches from the code
// that derives them -- rule:state-and-decisions, "name the command that derives
// the number, not the number". Run `propagate doctor` and read the line.
//
// The counts stay separate because they are different facts: no description means
// the skill cannot autotrigger AT ALL; a non-WHEN description merely matches
// requests less well.
try {
  ...
  // Third-party = the skill directory resolves outside ~/.claude/skills, i.e. it
  // is a symlink into ~/.agents/skills or a plugin cache. realpathSync, because
  // scanSkills hands back the symlink path and the whole point is to tell the two
  // apart. A skill that cannot be resolved is counted as OURS -- the conservative
  // side, since under-reporting someone else's problem is better than silently
  // excusing our own.
  const { realpathSync } = await import("node:fs");
  const isThirdParty = (sk) => {
    const raw = sk?.path || sk?.dir || sk?.file || "";
    if (!raw) return false;
    try {
      return /plugins\/cache|\/\.agents\/|node_modules/.test(realpathSync(raw));
    } catch {
      return false;
    }
  };
  const ours = list.filter((sk) => !isThirdParty(sk));
  const theirs = list.filter(isThirdParty);
  const noFmOurs = ours.filter((sk) => sk?.completeness?.frontmatter === false).map((sk) => sk.id);
  const noFmTheirs = theirs.filter((sk) => sk?.completeness?.frontmatter === false).map((sk) => sk.id);
  const notWhenOurs = ours.filter((sk) => sk?.completeness?.descriptionStatesWhen === false).length;
  const notWhenTheirs = theirs.filter((sk) => sk?.completeness?.descriptionStatesWhen === false).length;
  reporter.info(
    "skill frontmatter",
    `${list.length} scanned (${ours.length} ours, ${theirs.length} third-party)` +
      `; OURS: ${noFmOurs.length} with no description (cannot autotrigger)` +
      (noFmOurs.length ? `: ${noFmOurs.join(", ")}` : "") +
      `, ${notWhenOurs} not WHEN-phrased` +
      `; THIRD-PARTY (not yours to fix): ${noFmTheirs.length} with no description` +
      (noFmTheirs.length ? `: ${noFmTheirs.join(", ")}` : "") +
      `, ${notWhenTheirs} not WHEN-phrased`,
  );
} catch (err) {
