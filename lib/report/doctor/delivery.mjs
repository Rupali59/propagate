...
export async function checkDelivery({ reporter, repoRoot, cacheRoot }) {
  ...
  for (const t of trees) {
    ...
    if (verdict.state === "current") {
      ...
      // Label is the KIND, marketplace moves into the detail (slice 3).
      reporter.warn(
        "served plugin is STALE",
        `${label} — ${verdict.why} — run \`claude plugin update propagate@${t.marketplace}\`, then RESTART. ` +
          `A running session keeps the old code.`,
      );
    } else {
      reporter.warn(
        "served plugin DIVERGED from source",
        `${label} — ${verdict.why}. Either this tree has uncommitted edits (normal while developing) ` +
