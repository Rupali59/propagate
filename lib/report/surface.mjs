 ...
 */
export const SERVED_VIEWS = Object.freeze(["/queue", "/issues", "/todos"]);
...
export function buildRegisterRows(reg, gotchas) {
  ...
  if (gotchas) {
    rows.push({
      ...
      // TONE STAYS OK WHILE ANY ENTRY CAN FIRE, and that is deliberate rather
      // than lenient. `rule:every-project-carries-gotchas` is explicit that most
      // gotchas have NO mechanical trigger and that inventing one makes noise —
      // so a partly-triggerless file is the designed state, not a defect. Only a
      // file where NOTHING can fire is a hazard documented and not delivered.
 ...
 */
export function buildSurface({ queue, snapshot, registers, gotchas, now = Date.now(), generatedAt, served = SERVED_VIEWS } = {}) {
