...
function headingsOutsideFences(text, re) {
  // ROUTED THROUGH lib/docs/tokens.mjs 2026-09-24 (PR-003 3b). This was the third
  // and last independent copy of the fence-aware scan; handovers.mjs and caps.mjs
  // were the other two, and caps.mjs's copy turned out to cover only one of the
  // two fence forms.
  //
  // It takes the FENCE SET rather than the heading list, deliberately: the
  // original tested every unfenced line, not only headings, and both callers pass
  // heading-shaped regexes. Filtering to headings first would be a behaviour
  // change dressed as a refactor. `fenced` carries the delimiters AND the
  // interior, which is exactly the set the old loop skipped.
  const { lines, fenced } = scanStructure(text);
  return lines.filter((line, i) => !fenced.has(i + 1) && re.test(line));
}
