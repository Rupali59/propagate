...
export function notifyKey(row) {
  // A pre-computed key wins. Backlog defects arrive from `backlogDefects()`
  // already carrying a stable `key` (`backlog:<kind>:<path>`) because their
  // identity is the FILE, not a pair of content hashes — a defect should notify
  // once and stay quiet until it is fixed, not re-fire whenever the file's
  // bytes change. Honouring it here means defects and edges share one dedupe
  // path, one store and one cap, instead of a second set of machinery that can
  // drift from this one.
...
/**
 * Backlog defects worth telling someone about, minus the ones already sent.
 *
 * SEPARATE FROM `selectToNotify` because actionability is decided differently
 * and deliberately so: an edge must pass `isActionable(state)`, while a defect
 * IS the finding — `backlogDefects()` has already excluded everything that is
 * merely open work. Reusing the edge filter here would silently drop every
 * defect, since none of them carry a `state`.
 *
 * @param {Array<{key: string}>} defects - backlogDefects() output
 * @param {Set<string>} alreadyNotified - keys from readNotified()
 */
export function selectDefectsToNotify(defects, alreadyNotified = new Set()) {
  const toNotify = [];
  const suppressed = [];
  for (const d of defects ?? []) {
    if (alreadyNotified.has(d.key)) suppressed.push(d);
    else toNotify.push(d);
  }
  return { toNotify, suppressed };
}

/**
 * The notification body for backlog defects. Names files, like the edge
 * formatter names edges — "3 problems" sends nobody anywhere.
 */
export function formatDefectNotification(defects, opts = {}) {
  const root = opts.root ? (opts.root.endsWith("/") ? opts.root : opts.root + "/") : "";
  const rel = (p) => (root && p && p.startsWith(root) ? p.slice(root.length) : p || "(unknown)");
  const shown = defects.slice(0, 3);
  const lines = shown.map((d) => `${d.kind} ${rel(d.file)}`);
  if (defects.length > shown.length) lines.push(`+${defects.length - shown.length} more`);
  const title =
    defects.length === 1
      ? "propagate: 1 backlog file holds work no count can see"
      : `propagate: ${defects.length} backlog files hold work no count can see`;
 ...
 */
/**
 * A path short enough for a notification and still unambiguous.
 *
 * WHY NOT `path.basename`. It was, and in this tree that is barely better than
 * the bare count this function exists to avoid. Measured 2026-09-17 over 44
 * actionable rows: `CLAUDE.md → CLAUDE.md` is the correct rendering of **seven
 * different edges**, because every workspace has a `CLAUDE.md` (53 of them) and
 * most carry a `STATE.md`, `GOTCHAS.md` and `DECISIONS.md` too. The notification
 * was accurate and unusable — "I see something drifting, but can't see what".
 *
 * The first path segment is the workspace, which is the discriminator that
 * matters; the middle is elided because it is the part that makes a line too
 * long to read in a banner.
 */
function shortPath(rel) {
  if (!rel || rel === "(glob)") return "(glob)";
  const parts = rel.split("/").filter(Boolean);
  if (parts.length <= 3) return parts.join("/");
  // Workspace + PARENT DIRECTORY + filename. Keeping the parent is not cosmetic:
  // a first version elided to `workspace/…/file` and, run against the real
  // actionable set, rendered two different edges identically as
  // `Vipin Kaushik/…/CLAUDE.md` — the discriminator between
  // `VipinKaushik/CLAUDE.md` and `astroacharya/CLAUDE.md` is exactly the segment
  // it dropped. Both ends of an edge frequently live in ONE workspace, so the
  // workspace alone separates nothing there.
...
export function formatNotification(toNotify, opts = {}) {
  ...
  const lines = shown.map((r) => `${r.state} ${shortPath(rel(r.source.path))} → ${shortPath(rel(r.downstream.path) || "glob")}`);
