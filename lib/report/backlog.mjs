 ...
 *
 * A SEPARATE axis, per-ITEM rather than per-FILE: within a checkbox or
 * id-keyed file, each item is `open`, `closed`, or `proposed` -- an entry
 * staged under an unreviewed heading (`PROPOSED_SECTION_RE`), not yet
 * triaged. `proposed` is excluded from BOTH `open` and `closed`; three
 * different facts, never conflated (rule:discernment-checks §2). See
 * `proposedSectionLines` and its call sites in `parseTodoLikeFile` below.
 */
...
export function discoverBacklogFiles({ searchRoots = SEARCH_ROOTS, maxDepth = MAX_WALK_DEPTH, budgetMs = WALK_BUDGET_MS } = {}) {
  ...
  // goalsMd added 2026-09-14 for `propagate goals`. Added HERE rather than in a
  // second walk, for the reason stated above: two forks of one traversal is the
  // defect the size-caps review was about.
  const found = { stateMd: [], todosMd: [], issuesMd: [], handoverMd: [], claudeMd: [], gotchasMd: [], memoryMd: [], goalsMd: [] };
  const dropped = [];
  ...
  function walk(dir, depth) {
    ...
    // GOALS.md — arrival conditions, the fourth file beside STATE / DECISIONS /
    // GOTCHAS. Single path only, unlike ISSUES.md and GOTCHAS.md above: this
    // file is new as of 2026-09-14, so there is no pre-v3 location for it to
    // have been left behind in. If that ever stops being true, add the second
    // path rather than swapping this one.
    if (existsSync(path.join(dir, "GOALS.md"))) found.goalsMd.push(path.join(dir, "GOALS.md"));
...
const CLOSED_SECTION_RE =
  /\b(archiv|resolved|done|cancelled|canceled|closed|complete|finish|shipped|superseded|landed)/i;

// The reader's THIRD state (plan: "i-saw-it-what-starry-sparkle", Part 1 / R1,
// approved 2026-09-25). The register inserter (`lib/registers/insert.mjs`,
// a separate lane) stages a reminder-derived entry under an unreviewed
// heading rather than into open work — Rupali's own call, recorded
// `propagation/state/workspace/DECISIONS.md:1187-1219`: "insert into a
// staging section ... rather than into open work." Before this, that heading
// was cosmetic: `parseTodoLikeFile` had no third bucket, so a staged entry
// either vanished (no id — invisible) or counted OPEN (id-keyed — inflating
// backlog before a human ever looked at it). `## From Reminders (unreviewed)`
// is the worked example from the plan.
//
// Deliberately unscoped, per R1's accepted answer (D3, against the C
// recommendation to opt in per file): this fires WHEREVER it matches across
// the corpus, not only in files a register inserter has touched. The
// accepted risk — a heading elsewhere in the 107-file corpus matching for an
// unrelated reason and silently reclassifying real open work — is exactly
// what the R4 corpus-regression test (`tests/unit/backlog-proposed-corpus.test.mjs`)
// exists to catch and name, not to prevent by construction.
const PROPOSED_SECTION_RE = /\b(unreview|staging|staged|propos)/i;
...
export function closedSectionLines(lines) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (h) {
      ...
      if (closedDepth === null && CLOSED_SECTION_RE.test(h[2])) {
        closedDepth = depth;
        return; // the declaring heading itself is not an item
      }
      // A NESTED HEADING INSIDE A CLOSED SECTION IS A CLOSED ITEM. Added
      // 2026-09-24: this branch used to `return` unconditionally, so headings
      // were never added to the set — which meant the "closed section wholesale"
      // mechanism worked for checkbox items and NEVER for ID-keyed ones, whose
      // items ARE headings. Both propagate's and Keerti's TODOS.md headers tell
      // the reader that moving an entry under `## Finished` closes it; for an
      // id-keyed file that was false, and moving PR-001 there left it counted
      // open. N51's family: prose about the format is not the format.
      if (closedDepth !== null && depth > closedDepth) out.add(idx + 1);
      return;
    }
...
/**
 * Line numbers (1-indexed) that sit under a heading declaring its section
 * PROPOSED — an unreviewed staging area, not yet triaged into open work.
 *
 * Deliberate MIRROR of `closedSectionLines` above, structure for structure,
 * including its 2026-09-24 branch: a heading nested inside a proposed section
 * is itself a proposed ITEM (`depth > proposedDepth` is added to the set, not
 * skipped), because an id-keyed register's items ARE headings — the same
 * fact that made `closedSectionLines` wrong for id-keyed files until that
 * branch existed (N51's family: prose about the format is not the format).
 *
 * A separate walk and a separate depth tracker from `closedSectionLines`,
 * not a shared one: a file can carry both a `## Closed` and a
 * `## From Reminders (unreviewed)` section, at different heading levels,
 * open at the same time, and each must close on its own next same-or-shallower
 * heading — not on the other section's.
 */
export function proposedSectionLines(lines) {
  const out = new Set();
  let proposedDepth = null;
  lines.forEach((line, idx) => {
    const h = line.match(/^(#{1,6})\s+(.*)$/);
    if (h) {
      const depth = h[1].length;
      if (proposedDepth !== null && depth <= proposedDepth) proposedDepth = null;
      if (proposedDepth === null && PROPOSED_SECTION_RE.test(h[2])) {
        proposedDepth = depth;
        return; // the declaring heading itself is not an item
      }
      if (proposedDepth !== null && depth > proposedDepth) out.add(idx + 1);
      return;
    }
    if (proposedDepth !== null) out.add(idx + 1);
  });
  return out;
}
...
// How a pointer stub names its target. Ordered: an explicit declaration beats
// prose. Measured 2026-08-28 across 57 real stubs (classified by
// isPointerStubText, not by grepping for the words "pointer stub" — doing that
// wrongly counts real files that merely MENTION stubs, which is how this
// measurement was first taken and why it was wrong).
//
// The single `now lives at \`path\`` pattern this replaced found 47 and missed
// **10**, every one of which named its target perfectly well in different words.
// The stubs were not the defect; the reader was. "(target not named)" was
// printed about files whose third line is the path — the same shape as N56 one
// level down: a reader that cannot read its input reports ABSENCE, and absence
// is the reading people act on (rule:discernment-checks §6).
//
// The five phrasings actually in the tree, each with a live example:
//   | target | `path` |            Motherboard/STATE.md          (a table row)
//   now lives at [`path`](path)    the hub's own STATE.md         (markdown link)
//   The file lives at\n`path`      Tathya/STATE.md                (no "now", newline)
//   The file is at\n`path`         Tathya/WorkTracker/STATE.md    ("is", not "lives")
//   moved to **`path`**            VipinKaushik/TODOS.md          (bold-wrapped)
const STUB_TARGET_PATTERNS = [
  // An explicit `| target | `path` |` row is a declaration, not prose. First,
  // so a file carrying both cannot have the prose win.
  // `[>\s]*` because the row is often inside a blockquote — Motherboard/STATE.md
  // renders its whole stub as one `>` block, and anchoring on `^\s*` missed it.
  /^[>\s]*\|\s*target\s*\|\s*`([^`]+)`\s*\|/im,
  // Prose. The path may be a bare code span, a [`code`](link), or bold-wrapped.
  /(?:lives?|is|moved)\s+(?:at|to)\s+\*{0,2}\[?`([^`]+)`/i,
];

/**
 * The path a pointer stub names, for the reader who wants to follow it.
 *
 * Only ever called on text isPointerStubText() has already accepted, which
 * bounds the input to a short signpost — that is what makes a pattern as loose
 * as `is at` safe here. It is still guarded: a capture is only returned if it
 * LOOKS like a path to a state file. Naming the wrong path would be worse than
 * naming none, because a reader would follow it.
 */
function pointerStubTarget(text) {
  for (const re of STUB_TARGET_PATTERNS) {
    const m = text.match(re);
    if (!m) continue;
    const candidate = m[1].trim();
    if (!/\.md$/i.test(candidate)) continue; // not a target; keep looking
    return candidate;
  }
  return null;
}
...
export function parseTodoLikeFile(text, filePath) {
  ...
  const inProposedSection = proposedSectionLines(lines);
  ...
  lines.forEach((line, idx) => {
    ...
    if (m) {
      // PROPOSED wins over closed: an item under an unreviewed heading has
      // not been triaged, whatever its checkbox mark says. `proposed`,
      // `open` and `closed` are three different facts (rule:discernment-
      // checks §2) — a proposed entry must be none of the other two, so
      // `closed` is gated on `!proposed` rather than computed independently.
      const proposed = inProposedSection.has(idx + 1);
      checkboxMatches.push({
        lineNumber: idx + 1,
        proposed,
        closed: !proposed && (m[1].toLowerCase() === "x" || inClosedSection.has(idx + 1)),
        text: m[2].trim(),
      });
    }
  });
  if (checkboxMatches.length > 0) {
    const proposedCount = checkboxMatches.filter((c) => c.proposed).length;
    const items = checkboxMatches
      .filter((c) => !c.closed && !c.proposed)
    ...
    return {
      ...
      open: items.length,
      closed: checkboxMatches.length - items.length - proposedCount,
  ...
  lines.forEach((line, idx) => {
    ...
    if (hm) {
      ...
      // Same precedence as the checkbox branch: proposed excludes closed,
      // never the reverse.
      const proposed = inProposedSection.has(idx + 1);
      const closed =
        !proposed &&
        (CLOSED_MARKERS_RE.test(headingText) || CLOSED_MARKERS_RE.test(nextNonEmpty) || inClosedSection.has(idx + 1));
      idMatches.push({ lineNumber: idx + 1, id: hm[2], closed, proposed, text: headingText });
      return;
    }
    ...
    if (bm) {
      ...
      const proposed = inProposedSection.has(idx + 1);
      const closed = !proposed && (struckThrough || CLOSED_MARKERS_RE.test(bulletText) || inClosedSection.has(idx + 1));
      idMatches.push({ lineNumber: idx + 1, id: bm[2], closed, proposed, text: bulletText });
    }
  });
  if (idMatches.length > 0) {
    const proposedCount = idMatches.filter((c) => c.proposed).length;
    ...
    return {
      ...
      open: items.length,
      closed: idMatches.length - items.length - proposedCount,
      proposed: proposedCount,
      unparsed: null,
      stub: false,
      items,
  ...
  // `proposed: null` for the same reason `open`/`closed` are null here: an
  // unrecognised file makes no claim about any of the three states, and 0
  // would read as a claim ("nothing proposed here") rather than "could not
  // look".
  return {
    file: filePath,
    format: "unrecognised",
...
/**
 * Exported 2026-08-31 for the rollup module (`lib/report/rollup.mjs`), which
 * needs to check an EXPECTED register path that this walk may or may not have
 * reached (bounded by MAX_WALK_DEPTH/WALK_BUDGET_MS) without duplicating this
 * function's error-shape contract. Behaviour is unchanged for every existing
...
/**
 * Hash the TEXT `readTextSafe` already read -- zero additional IO, per the
 * rollup plan (`docs/plans/…rollup…md`, task brief Task 1). Hashing the
 * decoded UTF-8 string (re-encoded to UTF-8 bytes) rather than reading the
 * file's raw bytes a second time is a deliberate, bounded approximation:
 * for any file that IS valid UTF-8 -- every real register in this tree --
 * `Buffer.from(text, "utf8")` reproduces the exact bytes on disk, so nothing
 * is lost. Re-reading the file to hash its raw buffer would double the IO
 * this field exists to avoid.
 *
 * DELIBERATELY NOT `lib/edges/content-id.mjs`'s `contentId()`: that function
 * resolves a git repo FIRST (`findRepoRoot`) and returns
 * `{ unresolvable: "no-repo" }` for anything outside one. The rollup this
 * feeds hashes registers across workspaces that are not git repos at all
 * (doc-only husks like `Khushboo/`, `Rishabh/`) -- a hasher that refuses
 * outside a repo would silently blank every one of them. Do not "unify"
 * these two hashing paths later; they answer different questions
 * (content-addressing FOR a commit vs. drift-detection independent of one).
 */
 ...
 *   - `totals`      -- { parsedFiles, unparsedFiles, stubFiles, parsedItems, proposedItems, unparsedFileList }
...
export function backlog({ searchRoots } = {}) {
  ...
  const stateFiles = discovery.stateMd.map((f) => {
    ...
    // ADDITIVE (task brief Task 1): `sha256` is a NEW key on every record.
    // Every existing branch below is untouched except for the one extra key
    // appended at each return -- `discoverBacklogFiles`'s own comment blesses
    // this shape ("existing callers read only the keys they know, so widening
    // the result cannot change backlog's behaviour"). Three, and only three,
    // outcomes for this field: a 64-hex digest when the text was read; `null`
    // when `readTextSafe` reported an error (the `error` key already carries
    // the reason, so `sha256: null` here is redundant-but-explicit, never a
    // fourth undocumented state); and simply ABSENT as a key when the file was
    // never in the walk's result at all (a fact `rollup.mjs` establishes by
    // checking file EXISTENCE, not by inspecting this array for a missing sha).
    if (error) return { file: f, error, format: "unreadable", items: [], sha256: null };
    ...
    return { file: f, error: null, format: "state-live-sections", items, sha256: sha256OfText(text) };
  });
  ...
  function parseGeneric(files) {
    return files.map((f) => {
      const { text, error } = readTextSafe(f);
      if (error) {
        return { file: f, format: "unreadable", parsed: null, unparsed: `unreadable: ${error}`, stub: false, items: [], sha256: null };
      }
      // Same additive contract as stateFiles above: `parseTodoLikeFile`'s
      // return shape (checkbox / id-keyed / pointer-stub / stub / unrecognised)
      // is untouched; `sha256` is spread in from OUTSIDE it, over the text this
      // function already holds, so parseTodoLikeFile itself never needs to know
      // hashing exists.
      return { ...parseTodoLikeFile(text, f), sha256: sha256OfText(text) };
    });
  }
  ...
  // ADDITIVE aggregate for the reader's third state (R1): entries under an
  // unreviewed heading, excluded from `parsedItems`/`open` above. `?? 0`
  // because `unrecognised` reports `proposed: null` (G2 — no claim, not a
  // zero claim) and must not silently count as zero staged work either.
  const proposedItems = genericFiles.reduce((sum, f) => sum + (f.proposed ?? 0), 0);
