...
import { scanSkills, probeTranscripts, PROJECTS_DIR } from "../skills/skills-scan.mjs";
import { SEARCH_ROOTS, INTEGRATIONS } from "../core/config.mjs";
import { resolveBin } from "../core/which.mjs";
import { CLAUDE_HOME, CLAUDE_SETTINGS, CLAUDE_DAILY_MD } from "../core/paths.mjs";

/**
 * The shell command a human can run to check whether an artifact is still referenced.
 *
 * This is GENERATED CONTENT — written into inventory output and read by someone on
 * some machine. It used to interpolate `~/Documents/GitHub ~/.claude` as a literal,
 * so every probe this tool emitted told the reader to search a tree that exists on
 * one laptop. A probe that cannot run is worse than no probe: it looks actionable.
 *
 * Roots come from SEARCH_ROOTS (env > config.yml > default) plus the Claude home,
 * de-duplicated. Exported so a test can assert the emitted string without building an
 * inventory.
 */
export function livenessProbeFor(filePath) {
  const roots = [...new Set([...SEARCH_ROOTS, CLAUDE_HOME])];
  const quoted = roots.map((r) => (/\s/.test(r) ? JSON.stringify(r) : r)).join(" ");
  return `rg -l -F "${path.basename(filePath)}" -g '*.md' ${quoted}`;
}

export const SETTINGS_PATH = CLAUDE_SETTINGS;
export const DAILY_MD_PATH = CLAUDE_DAILY_MD;
...
/**
 * Nulls are FILTERED, not carried.
 *
 * `INTEGRATIONS.portsFile` resolves to null whenever its path does not exist —
 * that is `pick()` degrading gracefully, and it is correct. What was not correct
 * was spreading the null in here: `inventoryStandalone` then emitted
 * `standalone:null` with the evidence "file does not exist at null", a row about
 * nothing that reads like a finding, and called `existsSync(null)`, which raises
 * Node's [DEP0187] and is slated to throw outright in a later release.
 *
 * "Not configured" and "configured but missing" are different facts and neither
 * is a row. An unresolved integration is simply absent from the seed list.
 */
export const STANDALONE_SEEDS = [DAILY_MD_PATH, PORTS_YML_PATH].filter(
...
export function countReferrers(filePath, { roots = [...SEARCH_ROOTS, CLAUDE_HOME] } = {}) {
...
export function inventoryStandalone({ seeds = STANDALONE_SEEDS, roots } = {}) {
  ...
  for (const filePath of seeds) {
    ...
    items.push({
      ...
      evidence,
      artifacts: filePath,
      liveness_probe: livenessProbeFor(filePath),
    });
  }
