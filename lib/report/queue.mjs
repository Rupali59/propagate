...
export async function historyByEdge() {
  ...
  for (const e of events ?? []) {
    ...
    const h = map.get(e.edge_id) ?? { total: 0, noChange: 0, last: null, dispositions: [] };
    h.total += 1;
 ...
 *
 * `declared` is carried so a consumer can say "nothing actionable" WITHOUT it
...
export async function queuePayload(opts = {}) {
  ...
  return { items, summary: queueSummary(items), declared: rows.length, generatedAt: new Date().toISOString() };
}
