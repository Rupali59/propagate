...
import { scanStructure } from "../docs/tokens.mjs";
 ...
 * report `status: "exempt"`, which is not a pass — it means measured and
...
/**
 * WHICH KINDS THE GATE BLOCKS ON — narrowed 2026-08-28, and this is the whole
 * point of the distinction.
 *
 * The ratchet originally covered every capped kind. Measured, that was aimed at
 * files nobody pays for. Per session the tree auto-injects 1,631 lines: global
 * CLAUDE.md (138), hub CLAUDE.md (244), rules/*.md (1,246) and MEMORY.md (3).
 * STATE.md, TODOS.md, ISSUES.md, handovers and GOTCHAS.md — 15,526 lines — load
 * NEVER, automatically. They are opened when someone chooses to open them.
 *
 * And caps could not have fixed the on-demand cost anyway: the corpus is already
 * 43% under aggregate cap and still 568 KB. 51 STATE.md at exactly 200 lines is
 * 10,200 lines. FILE COUNT is the driver, not per-file size, so that is a
 * summarisation problem (`backlog --brief`) and not a gate.
 *
 * So the gate keeps only what is genuinely paid for on every session start.
 * Everything else stays in `capsReport` — knowing is free — but does not block.
 */
export const GATED_KINDS = new Set(["CLAUDE.md", "MEMORY.md"]);

/**
 * `rules/` is the highest-value cap in the tree and had none.
 *
 * 1,246 lines injected into EVERY session, growing, watched by nothing — 76% of
 * the entire per-session auto-load. Capped in AGGREGATE rather than per file
 * because `load-rules.mjs` reads the directory flat: eighteen 70-line rules cost
 * exactly what one 1,260-line rule costs, so a per-file cap would measure the
 * wrong unit and could be satisfied by splitting a file in two.
 *
 * 1400 gives ~12% headroom over the current 1,246 so a rule or two can still
 * land before the ratchet engages — the same "passable on day one" property that
 * stops any gate here teaching `--no-verify`. It is a chosen number, not a
 * derived one; re-derive the aggregate before changing it.
 */
export const RULES_DIR_CAP = 1400;
...
export function countEntries(text) {
  ...
  // ROUTED THROUGH lib/docs/tokens.mjs 2026-09-24 (PR-003 3b). The local strip was
  // WEAKER than the rule it defends, in the file whose comment cites N51:
  //   - it knew only \`\`\`, so a ~~~ fence was not a fence and its headings counted
  //   - it required a CLOSING fence, so an unterminated one stripped nothing and
  //     every heading after it counted
  // Both proven in tests/unit/caps-fences.test.mjs before this change. The
  // tokenizer handles both because handovers.mjs paid for them.
  return scanStructure(text).headings.filter((h) => h.depth === 3).length;
}
 ...
 *   exempt                — measured, deliberately uncapped (gotchas; D11 2026-08-28)
...
export function capsReport({ discover = discoverBacklogFiles, readFile = (f) => readFileSync(f, "utf8"), hubRoot = HUB_ROOT } = {}) {
  ...
  for (const file of [...new Set(files)]) {
    ...
    let status;
    if (cap === null && kind === "GOTCHAS.md") status = "exempt";
...
export function capsGate({ repoRoot, runGit, hubRoot = HUB_ROOT, bypass = false } = {}) {
  ...
  for (const rel of staged) {
    ...
    // NARROWED 2026-08-28. Report-only kinds are recorded and never blocked.
    // They stay fully visible in `capsReport`; the gate simply is not the right
    // instrument for a cost that is paid by choosing to open a file.
    if (!GATED_KINDS.has(kind)) {
      out.skipped.push({
        file: rel,
        notable: false,
        reason: `report-only kind (${kind}) — not auto-loaded, so not gated`,
      });
      continue;
    }
    ...
    const cap = capFor({ kind, scope });
    if (cap === null) {
      // A GATED kind carrying no declared cap — in practice a workspace-scope
      // MEMORY.md, which the caps table does not name.
      //
      // GOTCHAS.md can NO LONGER reach here: it short-circuits at the
      // GATED_KINDS check above. The gotchas-specific branch this carried went
      // dead on 2026-08-28 and is removed rather than left to mislead — and it
      // is worth naming that no test caught it, because dead code is exactly
      // what tests cannot reach.
      //
      // `notable` is true here: a gated kind we cannot judge is information,
      // unlike the report-only skips above, which are the common case and stay
      // quiet so they do not become noise (propagate GOTCHAS G23).
      out.skipped.push({
        file: rel,
        notable: true,
        reason: `no cap declared for ${scope}/${kind} — gated kind, unjudged`,
      });
      continue;
    }
  ...
  // rules/ is capped in AGGREGATE, so it cannot be judged one staged file at a
  // time. Only compute it when a rules file is actually in the commit — the
  // common commit touches none and pays nothing.
  const stagedRules = staged.filter((f) => /^rules\/[^/]+\.md$/.test(f) && !f.split("/").pop().startsWith("_"));
  if (stagedRules.length) {
    let allRules = [];
    try {
      allRules = runGit(["ls-files", "rules/"]).split("\n")
        .filter((f) => /^rules\/[^/]+\.md$/.test(f) && !f.split("/").pop().startsWith("_"));
    } catch {
      out.skipped.push({ file: "rules/", notable: true, reason: "could not list rules/ — aggregate NOT checked" });
      return out;
    }
    const sum = (ref) => {
      let n = 0;
      for (const f of allRules) {
        try { n += countLines(runGit(["show", `${ref}${f}`])); } catch { /* absent at this ref */ }
      }
      return n;
    };
    const now = sum(":");        // the index, i.e. what this commit would land
    const before = sum("HEAD:");
    if (now > before && now >= RULES_DIR_CAP) {
      out.blocked.push({ file: "rules/ (aggregate)", kind: "rules-dir", scope: "hub", cap: RULES_DIR_CAP, before, now, added: now - before });
    } else {
      out.allowed.push({ file: "rules/ (aggregate)", cap: RULES_DIR_CAP, before, now, reason: now < RULES_DIR_CAP ? "under cap" : "over cap but did not grow" });
    }
  }
