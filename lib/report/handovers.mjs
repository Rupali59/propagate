...
import { scanStructure, markerInWindow } from "../docs/tokens.mjs";
...
/**
 * Fence handling moved to `lib/docs/tokens.mjs` 2026-09-24 (PR-003 3b), which
 * now owns the heading scan, the fence rule and the marker window for every
 * register reader. The N51 record and the reason fenced lines CONSUME the window
 * without being matched live with the code there, in one place, rather than in a
 * second copy here.
 *
 * MARKER_WINDOW above stays: the window LOGIC is centralised, but its VALUE
 * belongs to this document kind and is passed in.
 */
...
export function parseHandovers(filePath) {
  ...
  // ROUTED THROUGH lib/docs/tokens.mjs 2026-09-24 (PR-003 3b). This loop used to
  // hand-roll the fence tracking, heading scan and marker window the tokenizer
  // now owns. It was the ONLY other module in lib/ carrying a window, so this
  // takes that logic from two copies to one. The bounds are unchanged because
  // they came from HERE: MARKER_WINDOW and the fenced-lines-consume-the-window
  // rule are this module own paid-for knowledge, extracted rather than invented.
  const struct = scanStructure(lines.join("\n"));
  const fileDate = (FILE_DATE_RE.exec(path.basename(filePath)) || [])[1] || null;

  for (const h of struct.headings) {
    const m = SECTION_RE.exec(lines[h.line - 1]);
    if (!m) continue; // a heading, but not a dated section heading
    close();
    current = {
      date: m[1] || fileDate,
    ...
    const d = markerInWindow(lines, h.line, MARKER_WINDOW, DONE_WHEN_RE);
    if (d) current.doneWhen = d.value;
    const r = markerInWindow(lines, h.line, MARKER_WINDOW, RESOLVED_RE);
    if (r) current.resolved = r.value;
  }
  close();
