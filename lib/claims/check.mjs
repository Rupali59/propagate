 ...
 */
export function findExpiredDates(text, { now = new Date(), counters = null } = {}) {
  const findings = [];
  ...
  const record = (lineNo, line, marker, dateText, approx, expiredStamp) => {
    ...
    findings.push({
      ...
      daysExpired,
      // PER-FINDING, tested against this finding's OWN line. The suppression
      // counters below count LINES across the whole file — a different unit.
      // Mixing them published "17 not past-tense" where the like-for-like
      // answer was 48 (`rule:discernment-checks` §5, 2026-09-10). Anything
      // compared against `findings.length` must use this flag, never those.
      looksHistorical: HISTORICAL_RECORD_RE.test(line),
    });
  };
  ...
  // So it goes to a CALLER-SUPPLIED object instead. Until 2026-09-10 this line was
  // `void suppressed` and the number was computed and thrown away, which left the
  // contract stated in HISTORICAL_RECORD_RE's comment ("`claims check` reports every
  // passed date as a CANDIDATE and prints how many look historical") half-built: the
  // counting half shipped, the printing half had nothing to print. Measured on the
  // live tree that day, all 71 expired-date findings rendered identically while 68
  // matched HISTORICAL_RECORD_RE — so the check read as 71 defects and would have
  // been dismissed wholesale. That is `rule:enforcement-watches-itself` §4 arriving
  // through the front door instead of the back.
  //
  // This SUPPRESSES NOTHING and makes no judgment. Every candidate is returned
  // exactly as before; the caller merely learns how many carry the historical shape,
  // which is what lets a human triage instead of skim.
  if (counters) {
    counters.acknowledged = (counters.acknowledged || 0) + suppressed.acknowledged;
    counters.historical = (counters.historical || 0) + suppressed.historical;
  }
  return findings;
}
...
export async function claimsCheck({ workspaces = [], now = new Date() } = {}) {
  ...
  // Accumulates across every file so the renderer can report how many expired-date
  // CANDIDATES carry the past-tense shape. Diagnostic only — nothing is filtered.
  const expiredCounters = { acknowledged: 0, historical: 0 };
  const filesChecked = [];
  ...
  for (const [absPath, text] of textByPath) {
    ...
    for (const f of findExpiredDates(text, { now, counters: expiredCounters })) {
  ...
  return {
    ...
    // Every field named without `Lines` is a FINDING count, so they are
    // subtractable. The line-level counters keep a `scannedLines` prefix
    // because they count scanned lines across every file, not findings — the
    // two were subtracted from each other once already.
    expiredDateCandidates: (() => {
      const ed = findings.filter((f) => f.check === "expired-date");
      return {
        total: ed.length,
        looksHistorical: ed.filter((f) => f.looksHistorical).length,
        notHistorical: ed.filter((f) => !f.looksHistorical).length,
        scannedLinesHistorical: expiredCounters.historical,
        scannedLinesAlreadyAcknowledged: expiredCounters.acknowledged,
      };
    })(),
  };
}
