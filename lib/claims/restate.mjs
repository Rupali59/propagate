...
export function restatementPairs(corpus, rules, opts = {}) {
  ...
  for (const entry of corpus ?? []) {
    ...
    if (!rule) {
      unpaired.push({ ...entry, pairSha: unpairedSha(entry, null), reason: `rule "${entry.rule}" is not among the loaded rules` });
      continue;
    }
    ...
    if (!fact) {
      unpaired.push({ ...entry, pairSha: unpairedSha(entry, null), reason: `rule file unreadable or empty: ${rule.__file}` });
      continue;
    }
    ...
    if (fileText == null) {
      unpaired.push({ ...entry, pairSha: unpairedSha(entry, fact), reason: `restating file unreadable: ${entry.file}` });
      continue;
    }
    ...
    try {
      ...
      unpaired.push({ ...entry, pairSha: unpairedSha(entry, fact), reason: `rule fingerprint does not compile: ${err.message}` });
      continue;
    }

    const found = findClaimBlock(fileText, entry.line, re);
    if (!found.block) {
      unpaired.push({ ...entry, pairSha: unpairedSha(entry, fact), reason: found.reason });
      continue;
    }
