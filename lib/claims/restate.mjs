 ...
 * reported by `rules-check.mjs`) — walking FORWARD from a structure anchor
 * when needed.
 *
 * WHY THE FORWARD WALK (N72). `checkRules` reports the line where a rule's
 * fingerprint matched, and for a fingerprint like `tool-priority`'s that is
 * very often a section heading (`## MCP: code-review-graph`) or the HTML
 * comment above it — `splitBlocks` correctly classes both as structure, not a
 * claim. Giving up there is honest but throws away real information: a
 * heading is a LABEL for the prose beneath it, and measured against the live
 * tree, 2 of 10 "unpairable" entries carried an accurate restatement in the
 * very next block that this function never looked at. So when the anchor
 * itself is structure, walk forward a BOUNDED number of blocks (see
 * FORWARD_BOUND) looking for the next judgeable one and test the fingerprint
 * against THAT block instead of surrendering immediately.
 *
 * THE BOUND IS THE WHOLE SAFETY PROPERTY. An unbounded walk would eventually
 * pair a heading with whatever prose happens to follow somewhere later in the
 * file, which is worse than not pairing — it would manufacture a claim out of
 * an unrelated section. Two blocks past the anchor is enough to reach "the
 * paragraph right under this heading" (and to hop over one intervening
 * structure block, e.g. a comment sitting between the heading and its prose)
 * without reaching into the next unrelated section.
 *
 * WHAT THIS DOES NOT DO. It never widens what counts as judgeable — headings,
 * tables, comments stay structure (`blocks.mjs`'s `JUDGEABLE` set is
 * untouched) — and it never pairs a candidate whose text does not itself match
 * the fingerprint. A heading followed by a bare citation with no restated
 * content must still come back UNPAIRED: finding a judgeable block is
 * necessary but not sufficient, it also has to say the thing the rule says.
 *
...
function findClaimBlock(fileText, line, fingerprintRe) {
  ...
  const anchorIdx = blocks.findIndex((b) => line >= b.startLine && line <= b.endLine);
  if (anchorIdx === -1) {
    return { reason: `no block covers line ${line} — the file may have changed since the corpus was computed` };
  }
  const anchor = blocks[anchorIdx];

  if (anchor.judgeable) {
    if (!fingerprintRe.test(anchor.text)) {
      return {
        reason:
  ...
  // The anchor is structure (heading, comment, table, code, or rule). Walk
  // forward a bounded number of blocks looking for the first judgeable one,
  // and require ITS text to match the fingerprint — the walk finds a
  // CANDIDATE, it does not waive the check.
  const FORWARD_BOUND = 2;
  for (let i = 1; i <= FORWARD_BOUND && anchorIdx + i < blocks.length; i++) {
    const candidate = blocks[anchorIdx + i];
    if (!candidate.judgeable) continue;
    if (!fingerprintRe.test(candidate.text)) {
      return {
        reason:
          `line ${line} falls inside a "${anchor.kind}" block; the next candidate ` +
          `(a "${candidate.kind}" block at line ${candidate.startLine}) does not restate the rule's ` +
          `fingerprint — a citation can exist with no restatement nearby, or the corpus entry is stale`,
      };
    }
  ...
  return {
    reason:
      `line ${line} falls inside a "${anchor.kind}" block, which is structure, not a claim, ` +
      `and no judgeable block restating the fingerprint follows within ${FORWARD_BOUND} blocks`,
  };
}
