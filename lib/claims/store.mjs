 ...
 * whole layer exists: the constitution doc mixes these five in one prose register and the
...
/**
 * ONE canonical spelling per file, on write and on read.
 *
 * Found end-to-end, not by a unit test: a verdict recorded under
 * `/tmp/x/DOC.md` was invisible to a lookup for `/private/tmp/x/DOC.md` — the
 * same file, because macOS resolves `/tmp` through a symlink. And the failure
 * did not present as a failure. `render` reported **"current — markers already
 * match the store"**, because "no verdicts found for this spelling" and "the
 * markers are up to date" were rendered identically. That is
 * `rule:discernment-checks` §2 exactly: absence must be attributable.
 *
 * `nearestOwner` already resolves through `realpathSync` for this reason and
 * says so in its own comment — "symlinks and worktree checkouts make the same
 * file reachable via more than one path". Worktrees make it routine here, not
 * exotic.
 *
 * Falls back to `path.resolve` when the file does not exist: a verdict may
 * legitimately outlive the document it was about (that is an ORPHANED verdict,
 * a state this store reports), and a throw here would make recording one
 * impossible.
 */
...
export async function appendClaim(claim, opts = {}) {
  ...
  const stamped = {
    ...claim,
    // Canonical spelling, always — see canonicalFile. Written, not merely
    // compared, so the store never holds two spellings of one document.
    file: canonicalFile(claim.file),
    claim_id: claimId,
    ts,
