...
import { existsSync, mkdirSync, readFileSync, writeFileSync, appendFileSync, readdirSync, realpathSync } from "node:fs";
import path from "node:path";
import { execFileSync } from "node:child_process";
import { readActiveLines } from "./active-lines.mjs";
...
/**
 * Which refs are merged into `baseRef`, decided by `git cherry`.
 *
 * NEVER `git branch --merged`, AND NEVER `base..ref`. These repos squash-merge:
 * the merged content lands on the base as a NEW commit whose parent is the
 * base, so the branch is not an ancestor and ancestry-based checks report it as
 * unique work. `ref-resolver.sh:21` recorded this and the port re-derived it
 * wrong anyway.
 *
 * Measured across 35 live refs before this was written:
 *   git cherry           35 agree with the shell registry, 0 disagree
 *   git branch --merged  differs on 7, each saying `unmerged` where the truth
 *                        is `merged`
 *
 * That is the conservative direction — false "this branch still carries work"
 * alarms rather than false safety — but it is still wrong, and a classifier
 * nobody trusts is a classifier nobody reads.
 *
 * THE CAP IS PART OF THE CONTRACT. A ref further than `maxCherry` commits from
 * base is reported in `unmeasured`, never in `merged`. Reporting "merged"
 * because we stopped looking is how a tool talks someone into deleting work.
 *
 * @returns {{merged: Set<string>, unmeasured: string[], error: string|null}}
 */
export async function mergedInto(repoRoot, baseRef, { refs = null, maxCherry = MAX_CHERRY } = {}) {
  const merged = new Set();
  const unmeasured = [];
  if (!baseRef) return { merged, unmeasured, error: "no base ref declared" };

  // The base must resolve. An unresolvable base makes every verdict below
  // meaningless, and "" is what ref-resolver.sh substitutes so the state reads
  // UNMEASURED rather than "not merged".
  try {
    await runGit(repoRoot, ["rev-parse", "--verify", "-q", baseRef]);
  } catch {
    return { merged, unmeasured, error: `base ref ${baseRef} does not resolve in this repo` };
  }

  let names = refs;
  if (!names) {
    try {
      const { stdout } = await runGit(repoRoot, ["for-each-ref", "--format=%(refname:short)", "refs/heads/"]);
      names = stdout.split("\n").map((l) => l.trim()).filter(Boolean);
    } catch (err) {
      return { merged, unmeasured, error: errMessage(err) };
    }
  }

  for (const name of names) {
    if (name === baseRef) continue;
    // A remote-only ref has no local head to cherry against; resolve it to its
    // remote-tracking name rather than skipping it, or every remote-only branch
    // silently reports unmeasured.
    let target = name;
    try {
      await runGit(repoRoot, ["rev-parse", "--verify", "-q", `refs/heads/${name}`]);
    } catch {
      target = `origin/${name}`;
    }
    try {
      const { stdout } = await runGit(repoRoot, ["cherry", baseRef, target]);
      const lines = stdout.split("\n").map((l) => l.trim()).filter(Boolean);
      if (lines.length > maxCherry) {
        unmeasured.push(name);
        continue;
      }
      // `+` marks a commit whose patch-id is NOT in base. None means merged —
      // including the squash case, where the patch landed under a different sha.
      if (!lines.some((l) => l.startsWith("+"))) merged.add(name);
    } catch {
      // One unreachable ref must not void the whole repo's verdicts.
      unmeasured.push(name);
    }
  }
  return { merged, unmeasured, error: null };
}
...
export async function buildSnapshot(repoRoot, opts = {}) {
  ...
  // includeRemoteOnly: the registry's job is every branch that EXISTS, and a
  // branch nobody checked out locally is still work. Without this the snapshot
  // under-counts by 12 on Vipin Kaushik and reports each as pruned.
  const { refs, error } = await enumerateRefs(repoRoot, { includeRemoteOnly: true });
  if (error) {
  ...
  let merged = new Set();
  let unmeasured = new Set();
  let mergeError = null;
  if (baseRef) {
    // Hand it the refs already enumerated rather than letting it re-walk
    // refs/heads. That walk would miss every REMOTE-ONLY ref — the exact
    // 12-ref blind spot that fabricated 12 prunes on 2026-08-24 (G27).
    const r = await mergedInto(repoRoot, baseRef, { refs: refs.map((x) => x.ref) });
    merged = r.merged;
    unmeasured = new Set(r.unmeasured ?? []);
    mergeError = r.error;
  }
  ...
  const out = refs.map((r) => ({
    ...
    // `upstream` is a REQUIRED field of the spec (docs/REFERENCE.md:106-110)
    // and this row omitted it entirely, so `foldRepoRefs` filled null and
    // every pushed branch read as unbacked. Two different questions, and the
    // row previously carried only the second:
    //   upstream        the NAME of the tracking branch, or null if none
    //   upstream_track  how far ahead/behind, "" when there is nothing to say
    ...
    //   same      this ref IS the active line — a branch is not merged into
    //             itself, and calling it `unmerged` would flag the deploy
    //             branch of every project as carrying work at risk
    //   merged    `git cherry` found no patch missing from base
    //   unmerged  it found at least one
    //   null      no base, the base did not resolve, cherry failed, or the ref
    //             is past MAX_CHERRY. UNMEASURED — never `unmerged`, and never
    //             `merged`, which is the direction that talks someone into
    //             deleting work. `merge_state_error` carries the reason.
    merge_state:
      activeLine != null && r.ref === activeLine
        ? "same"
        : !baseRef || mergeError || unmeasured.has(r.ref)
          ? null
          : merged.has(r.ref)
            ? "merged"
            : "unmerged",
...
/** Seconds between two ISO stamps; null when either is missing or unparseable. */
function windowSeconds(fromIso, toIso) {
...
export function diffSnapshots(prev, next) {
  ...
  for (const [name, project] of Object.entries(nextProjects)) {
    ...
    for (const [ref, row] of Object.entries(nextRefs)) {
      ...
      // Normalise both sides: a snapshot written before canonicalPath existed
      // still holds symlinked paths, and must not read as a change.
      const before_ = new Set((was.worktrees ?? []).map(canonicalPath));
      const after_ = new Set((row.worktrees ?? []).map(canonicalPath));
    ...
    for (const [ref, was] of Object.entries(prevRefs)) {
      ...
      events.push({
        ...
        head: was.head,
        // THE FIELDS THE VERDICT WAS DERIVED FROM, not only the prose. The
        // retired shell registry recorded these and queried them with jq;
        // keeping them in `evidence` alone would make `lost` vs `recoverable`
        // un-recheckable, which this module's own header forbids.
        merge_state_at_last_sighting: was.merge_state ?? null,
        upstream_at_last_sighting: was.upstream ?? null,
        upstream_track_at_last_sighting: was.upstream_track ?? null,
        // WHEN it could have happened. A prune found by comparing two snapshots
        // occurred somewhere BETWEEN them; stamping only `at` asserts it
        // happened at the later moment, which is false precision in a record
        // nobody can amend. `null` when the previous capture carried no
        // timestamp — never 0, which would claim instantaneous.
        window_seconds: windowSeconds(prev?.captured_at, at),
    ...
    const prevDet = new Set((before.detached_worktrees ?? []).map((d) => canonicalPath(d.path)));
    const nextDet = new Map((project.detached_worktrees ?? []).map((d) => [canonicalPath(d.path), d]));
    ...
    for (const d of before.detached_worktrees ?? []) {
      if (nextDet.has(canonicalPath(d.path))) continue;
      events.push({
        type: "worktree-removed",
...
/**
 * WHAT THIS INSTRUMENT CANNOT SEE, stated on every artifact it writes.
 *
 * Ported verbatim from `hygiene/branch-registry.sh`, which emitted it on every
 * run and which propagate replaced without inheriting it. The limitation is
 * inherited whether or not the sentence is: lifecycle is derived by comparing
 * two captures, so a ref that appears AND disappears between them leaves no
 * trace anywhere.
 *
 * It rides on the snapshot rather than living in a doc because the artifact is
 * what gets read later, usually by someone who will treat lifecycle.jsonl as
 * complete history. An undeclared blind spot is indistinguishable from no blind
 * spot — rule:discernment-checks §2, one level up from a silent zero.
 */
export const SNAPSHOT_BLIND_SPOT =
  "a ref created and deleted inside one window is invisible to this instrument";

/**
 * How far from base a ref may be before its merged-ness is reported UNMEASURED.
 * Mirrors `ref-resolver.sh:38`'s MAX_CHERRY. The point is not performance — it
 * is that a verdict nobody actually computed must never render as `merged`.
 */
const MAX_CHERRY = 500;
...
/**
 * A worktree path as the filesystem really names it.
 *
 * git reports the resolved path (`/private/var/...` on macOS) while a path
 * recorded earlier — by the shell registry, or by a caller — may carry the
 * symlink (`/var/...`). Comparing them raw makes ONE worktree look like a
 * removal plus an addition, which is a fabricated pair of lifecycle events in
 * an append-only log. Caught 2026-08-24 by a tmpdir fixture; the same shape
 * fires for any symlinked checkout path, not just macOS tmpdirs.
 *
 * Falls back to the input when the path no longer exists — a worktree that was
 * just removed cannot be realpath'd, and that is precisely the case we must
 * still be able to compare.
 */
...
function foldRepoRefs(flat, { activeLine = null } = {}) {
  ...
  for (const r of flat) {
    ...
    if (!r.ref) {
      detached.push({ path: canonicalPath(r.path), head: r.head });
      continue;
    }
    ...
    if (!target) {
      ...
      conflicts.push(`worktree ${canonicalPath(r.path)} is on ${r.ref}, which has no branch row`);
      continue;
    }
    const wtPath = canonicalPath(r.path);
    if (target.worktrees.includes(wtPath)) continue;
    target.worktrees.push(wtPath);
  }
...
export async function buildWorkspaceSnapshot(workspaceRoot, opts = {}) {
  const { now = null } = opts;
  // ACTIVE LINES COME FROM CONFIG, NOT FROM GIT. `is_active_line` records a
  // decision — VipinKaushik deploys from `production` while `main` sits archived
  // at a tag — and no probe of the repo can tell you that. Reading it here is
  // what stops a regenerated snapshot from emitting `null` for all 36 refs and
  // silently losing the 7 flags the previous file carried.
  //
  // `opts.activeLines` still wins when given, so a caller (or a test) can pin
  // them; otherwise they are read from the workspace's own convention file.
  const declared = opts.activeLines ?? readActiveLines(workspaceRoot);
  const activeLines = declared.lines ?? declared;
  const activeLinesReason = declared.reason ?? null;
  const { projects: found, skipped } = discoverProjectRepos(workspaceRoot);
  const projects = {};
  for (const { name, repoRoot } of found) {
    const activeLine = activeLines[name] ?? null;
    // C1e.1 — the base is `origin/<active line>`, exactly as ref-resolver.sh:58
    // derives it. There is no separate base config; the active line IS the base.
    //
    // THIS LINE IS THE WHOLE C1e DEFECT. It used to omit `baseRef`, so every
    // one of the 36 refs got `merge_state: null` while the shell registry it
    // replaced held `unmerged 18 · same 7 · merged 10`. The conversion was
    // reported as a no-op because populations and flag counts matched —
    // content was never compared.
    //
    // A base that does not resolve is NOT silently dropped: `mergedInto`
    // returns an error and every merge_state becomes null WITH a reason on
    // `merge_state_error`, which is a different fact from null-because-nobody-asked.
    const baseRef = activeLine ? `origin/${activeLine}` : null;
    const one = await buildSnapshot(repoRoot, { project: name, baseRef, activeLine, now });
    if (one.error) {
    ...
    const folded = foldRepoRefs(one.refs, { activeLine });
    projects[name] = {
      repo_root: repoRoot,
  ...
  return {
    ...
    // Stated on the artifact, not in a doc. See SNAPSHOT_BLIND_SPOT.
    blind_spot: SNAPSHOT_BLIND_SPOT,
    workspace_root: workspaceRoot,
    projects,
    skipped,
    // Why every is_active_line may be null, when it is. Without this a reader
    // cannot tell "this workspace declares no active lines" from "we forgot to
    // look" (rule:discernment-checks §2).
    ...(activeLinesReason ? { active_lines_reason: activeLinesReason } : {}),
  };
}
