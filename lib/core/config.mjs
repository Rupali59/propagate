...
import { HOME, CLAUDE_RULES, CLAUDE_SKILLS, expandHome, stateDir, STATE_DIR_DEFAULT } from "./paths.mjs";

// Re-exported so existing consumers of config.mjs that reach for the pure
// registry through it (rather than importing lib/core/paths.mjs directly)
// keep working. Per the plan: config.mjs imports paths.mjs and re-exports;
// no consumer of config.mjs changes shape as a result.
export { PATHS, PATHS_DIAGNOSTIC, requirePath } from "./paths.mjs";
 ...
 */
const CONFIG_ROOT = process.env.PROPAGATE_STATE_DIR || STATE_DIR_DEFAULT;
export const CONFIG_PATH = path.join(CONFIG_ROOT, "config.yml");

/**
 * The full set of top-level `config.yml` keys anything in this file reads.
 * `CONFIG` below only checked "is this a plain object" (rule:discernment-checks
 * §1 territory: a check that cannot fail is worse than no check) — no schema
 * validated the keys inside it, so `huRoot:` (a typo of `hubRoot:`) was accepted
 * and silently ignored. A registry whose whole promise is "keeps adding things"
 * cannot have adding-a-thing fail silently, so an unrecognised key now warns on
 * stderr naming itself and the recognised set — never throws, per this module's
 * degrade-not-throw contract.
 */
...
export const CONFIG = (() => {
  try {
    ...
    const unknown = Object.keys(parsed).filter((k) => !RECOGNIZED_CONFIG_KEYS.has(k));
    if (unknown.length) {
      console.error(
        `propagate: ${CONFIG_PATH} has unrecognised key(s): ${unknown.join(", ")} — ignored. ` +
          `Recognised: ${[...RECOGNIZED_CONFIG_KEYS].sort().join(", ")}`,
      );
    }
    return parsed;
  } catch (err) {
...
/**
 * Bounds for doctor's `# Delivery` gate (`lib/report/doctor/delivery.mjs`):
 * how many shipped-path commits, or how many days, the served plugin may lag
 * behind source before doctor FAILS rather than warns.
 *
 * EXPLICIT BUILT-IN DEFAULTS, ON PURPOSE (G24). A config key read with no
 * built-in fallback resolves to `undefined`/`null` the moment nobody has
 * written a `config.yml`, which on every machine but the author's is every
 * machine — and `null` is not "no limit", it is a value that fails every
 * numeric comparison silently, which is the delivery gate simply never firing
 * while doctor reports the section as if it had run. Same shape as G24's
 * canonical incident: a sentinel arriving by the door of "nobody configured
 * this" rather than "this was configured wrong".
 *
 * Values are the ones the plan's decision ledger (D5) derived from the two
 * recorded incidents: 2026-09-16 was four undelivered PRs; 2026-09-24 was 48
 * commits over 7 days. Both trip either bound; a developer in the normal
 * window between a VERSION bump and running `plugin update` trips neither.
 */
export const DELIVERY_MAX_COMMITS = (() => {
  const raw = process.env.PROPAGATE_DELIVERY_MAX_COMMITS ?? CONFIG.deliveryMaxCommits;
  const n = Number(raw);
  return Number.isFinite(n) && n >= 0 ? n : 10;
})();

export const DELIVERY_MAX_DAYS = (() => {
  const raw = process.env.PROPAGATE_DELIVERY_MAX_DAYS ?? CONFIG.deliveryMaxDays;
  const n = Number(raw);
...
export const INTEGRATIONS = (() => {
  ...
  return {
    ...
    telegramDir: pick("PROPAGATE_TELEGRAM_DIR", "telegramDir", path.join(CLAUDE_SKILLS, "telegram")),
    notifier: pick("PROPAGATE_NOTIFIER", "notifier"),
  };
})();

/**
 * One diagnostic string per INTEGRATIONS key — HUB_ROOT_DIAGNOSTIC's shape
 * (:150-159 above), generalised for the registry that never got one. This is
 * the gap N95 and PR-016 name directly: `INTEGRATIONS.marketplaceDir` is
 * `null` on an unconfigured machine and `lib/skills/skills-scan.mjs:464`
 * happens to guard its own call site, but nothing named the FIX anywhere —
 * exactly the sentinel failure G24 describes, arriving through INTEGRATIONS'
 * door instead of HUB_ROOT's.
 *
 * Mirrors `requirePath()`/`PATHS_DIAGNOSTIC` in lib/core/paths.mjs, which
 * INTEGRATIONS cannot use directly: it is hub- and discovery-derived
 * (underHub()), not a pure join, so it stays in this file per the plan (§2).
 */
export const INTEGRATIONS_DIAGNOSTIC = (() => {
  const envNames = {
    marketplaceDir: "PROPAGATE_MARKETPLACE_DIR",
    portsFile: "PROPAGATE_PORTS_FILE",
    deployFile: "PROPAGATE_DEPLOY_FILE",
    mongoFile: "PROPAGATE_MONGO_FILE",
    telegramDir: "PROPAGATE_TELEGRAM_DIR",
    notifier: "PROPAGATE_NOTIFIER",
  };
  const out = {};
  for (const [key, envName] of Object.entries(envNames)) {
    ...
    out[key] =
      typeof value === "string" && value.length
        ? `ok: ${value}`
        : `${key} is not configured — tried ${envName} and ${CONFIG_PATH} integrations.${key}. ` +
          `Fix: propagate setup --hub <path>, or set ${envName}`;
  }
  return out;
})();

/**
 * Returns INTEGRATIONS[key], or THROWS naming the fix. See requirePath() in
 * lib/core/paths.mjs, which this mirrors for the one nullable registry that
 * stays in this file. A null must never reach an unguarded existsSync
 * silently (G24, N95, PR-016 in one line) — callers that need an integration
 * rather than merely wanting to skip a feature when it is absent should use
 * this instead of reading INTEGRATIONS[key] directly.
 */
export function requireIntegration(key) {
  const value = INTEGRATIONS[key];
  if (typeof value === "string" && value.length) return value;
  throw new Error(
    `propagate: requireIntegration(${JSON.stringify(key)}) — ` +
      `${INTEGRATIONS_DIAGNOSTIC[key] ?? `unknown integration (not declared in INTEGRATIONS): ${key}`}`,
  );
}
...
/**
 * Resolve $PROPAGATE_STATE_DIR into a usable absolute directory, given the
 * PURE candidate `stateDir()` (lib/core/paths.mjs) already computed —
 * env-resolved-or-default, no I/O. This is the IMPURE half: it may
 * `mkdirSync` the candidate into existence, and it degrades to the same
 * default `stateDir()` returns when unset, never a crash.
 *
 ...
 * a logged warning and the default-location fallback, never a crash.
 */
function ensureStateDir(candidate) {
  ...
  const DEFAULT = STATE_DIR_DEFAULT;
  if (!raw) return candidate; // candidate already equals DEFAULT when unset
  try {
 ...
 * comment on ensureStateDir above. When this is `null`, every path below
 ...
 */
export const STATE_DIR = ensureStateDir(stateDir());
...
export const RULES_DIR = (() => {
  ...
  return CLAUDE_RULES; // the pure default, from lib/core/paths.mjs
})();
