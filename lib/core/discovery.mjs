...
/**
 * The empty-ledger Markdown body, byte-for-byte what `renderMarkdown`
 * (lib/edges/ledger.mjs) produces for a JSONL with zero rows. Duplicated
 * here rather than imported because discovery is synchronous and
 * dependency-light BY DESIGN (see module header — importing an async
 * module at the top of a sync, module-load-time walk risks exactly the
 * frontmatter.mjs deadlock already documented above). Since the JSONL this
 * accompanies is always empty at creation time, staying byte-identical to
 * the real renderer means the first real `renderMarkdown` call is a no-op
 * overwrite, not a content change — no row, no drift, nothing to reconcile.
 */
const EMPTY_LEDGER_MD = [
  "# Propagation Ledger",
  "",
...
function makeWorkspaceRecord(wsRoot) {
  ...
  if (existsSync(propagationJsonl)) {
    ...
    ledgerHadNoPriorFile = !existsSync(docsJsonl) && !existsSync(legacyJsonl);
