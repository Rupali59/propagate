...
/**
 * The deepest workspace whose root contains `absPath`, or `null` if none does.
 *
 * `docs/ISSUES.md` §A4's stated fix, verbatim: "extract the nearest-owner
 * assignment into a shared helper (`lib/discovery.mjs` is the natural home)".
 * Before this function existed the same reduce-by-longest-root logic was
 * copied three times — `cli.mjs`'s `assignSidecarsToWorkspaces`,
 * `commands/status.mjs`'s `nestedUnderOf`, and `lib/core/config.mjs`'s
 * `currentWorkspace` — and F7 in the rollup plan found a FOURTH, wrong,
 * version already in the wild: `groupForBrief`'s `workspaceOf` used
 * `String(f).split("/")[0]`, a bare string-prefix split that attributes
 * `Keerti/Keerti-portfolio/…` to `Keerti` and makes the nested workspace
 * disappear. This function exists so that mistake has nowhere left to recur.
 *
 * Containment is a PATH-COMPONENT test, not a string-prefix test — always via
 * `path.sep`, never a hardcoded `/`. `ws.root === "/hub/Keerti"` must not
 * match `/hub/Keerti-other/x.md`; only `path.sep`-anchored containment (or
 * exact equality) tells those apart. Ties resolve to the LONGEST root string
 * — the deepest, most-nested workspace among every candidate that contains
 * the path — matching all three existing copies' `.length >` reduce.
 *
 * `absPath` is resolved through `realpathSync` first, inside a try/catch that
 * falls back to the raw path on failure — the same reasoning
 * `assignSidecarsToWorkspaces` already documents: "symlinks and worktree
 * checkouts make the same file reachable via more than one path", so
 * ownership must be decided on the real location, not whichever of several
 * equivalent strings happened to be passed in. A file that vanished between
 * being found and being resolved (or a path that was never real to begin
 * with) falls back to the raw string rather than throwing — this function
 * must never throw for a bad or missing path, only return `null`.
 *
 * Unlike `nestedUnderOf` (`commands/status.mjs`), this INCLUDES the workspace
 * itself as a candidate owner — `nestedUnderOf` answers "nearest ANCESTOR"
 * (`o.root !== ws.root`) because it is asking whether one known workspace
 * nests under another; this answers "nearest OWNER, self included" for an
 * arbitrary file path, which is what a caller wants when it does not already
 * know which workspace it started in. They are deliberately not the same
 * function — do not repoint `nestedUnderOf` at this one.
 *
 * @param {string} absPath
 * @param {Array<{root: string, name?: string}>} workspaces
 * @returns {{root: string, name?: string}|null}
 */
export function nearestOwner(absPath, workspaces) {
  if (!absPath || !Array.isArray(workspaces) || workspaces.length === 0) return null;

  let real;
  try {
    real = realpathSync(absPath);
  } catch {
    real = absPath; // vanished, or never existed — fall back to the raw path
  }

  const owners = workspaces.filter(
    (ws) => real === ws.root || real.startsWith(ws.root + path.sep),
  );
  if (owners.length === 0) return null;

  return owners.reduce((best, ws) => (ws.root.length > best.root.length ? ws : best));
}
...
// lib/core/discovery.mjs -> propagate/ (two levels up). Same depth as
// lib/report/rollup.mjs, where this lived until 2026-09-23, so the "../.."
// is unchanged by the move — checked rather than assumed, because G60 is a
// relative specifier that moved between directories and resolved silently wrong.
const HERE = path.dirname(fileURLToPath(import.meta.url));
const MODULE_ROOT = path.resolve(HERE, "../..");

/** realpath, never throwing: a directory can vanish between discovery and use. */
...
/**
 * The census BOTH `rollup` and `doctor` answer conformance from.
 *
 * MOVED HERE 2026-09-23 (N87 slice 1). It was private to `rollup.mjs` while
 * `doctor` used `WORKSPACES` from `lib/core/config.mjs` instead, and the two
 * disagreed: `WORKSPACES` is MARKER-based, and a marker is something a
 * workspace only carries once it has ADOPTED the v3 layout. So the check for
 * "did you adopt the layout" was gated on having adopted it — a workspace that
 * never began was not reported as failing, it was not reported at all.
 * Measured that day: 20 marker-based entries, containing neither `propagate`
 * nor `Motion-Graphics`, which are exactly the two `rollup` calls
 * NON-CONFORMANT.
 *
 * The predicate here is VERSION CONTROL, not a marker: every depth-1 directory
 * under a search root carrying a `.git`. That is the honest answer to "is this
 * a distinct, attributable thing", and it cannot be gated on the property being
 * measured.
 *
 * Self-inclusion is unconditional and deliberate — `rule:enforcement-watches-itself`
 * in mechanical form, not dependent on the depth-1 scan happening to reach this
 * directory.
 *
 * @param {string[]} roots search roots
 * @param {Array<{root: string, name?: string}>} discoveredWorkspaces marker-based set, never dropped
 * @returns {Array<{root: string, name: string}>} superset of the input
 */
export function ownerCandidates(roots, discoveredWorkspaces) {
  ...
  // SELF-INCLUSION, and why it is CONTAINMENT-based rather than unconditional.
  // rule:enforcement-watches-itself requires propagate to appear in its own
  // census — but "unconditional" (how this read while it was private to
  // rollup.mjs) drags the developer's real repo into any caller whose roots are
  // a fixture tmpdir. Measured 2026-09-23: wiring doctor to this census with
  // unconditional self-inclusion turned 5 hermetic doctor tests red, because a
  // fixture workspace's population suddenly contained this non-conformant repo.
  //
  // Containment is not the depth-dependence rollup warned against: propagate is
  // included whenever it lies anywhere beneath a search root, at any depth. It
  // is excluded only when the caller is looking somewhere else entirely, which
  // is exactly when including it would be wrong.
  const selfReal = resolveOwnerRoot(MODULE_ROOT);
  const within = roots.some((r) => {
    const rr = resolveOwnerRoot(r);
    return selfReal === rr || selfReal.startsWith(rr.endsWith(path.sep) ? rr : rr + path.sep);
  });
  if (within) add({ root: MODULE_ROOT, name: path.basename(MODULE_ROOT) });
