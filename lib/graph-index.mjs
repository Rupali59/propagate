...
export function emitSqlite(model, dbPath) {
  ...
  const insEvent = db.prepare(
    "INSERT INTO event (event_id,edge_id,node_id,disposition,ts,reason) VALUES (?,?,?,?,?,?)",
  );
  for (const e of model.events)
    insEvent.run(e.event_id, e.edge_id, e.node_id, e.disposition, e.ts, e.reason);
...
SELECT ev.edge_id, ev.ts, ev.disposition, ev.reason, s.path AS source_path, t.path AS downstream_path
FROM event ev
