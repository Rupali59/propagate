 ...
 *
 * Design note: the plan this originally cited
 * (~/.claude/plans/okay-pln-these-out-zany-rain.md) was later overwritten by an
 * unrelated design, so that citation pointed at nothing. Plans that govern this
 * repo now live in docs/plans/ — see docs/plans/2026-08-19-portability-and-rules.md
 * for why. The design intent for THIS file is the two traps documented below.
 *
...
/**
 * Edge states, worst first. A node takes the worst state among its edges.
 *
 * LIVES HERE, NOT IN THE RENDERER. This was defined in lib/graph/graph-html.mjs
 * while `fixOrder` below sorted by `state.localeCompare` — so the model
 * ordered work by NAME and only the view knew what "worse" meant. Alphabetical
 * and severity agree on DIVERGED first and disagree immediately after:
 *
 *   alphabetical   DIVERGED, DRIFTED,  REVERSED, UNMATCHED
 *   severity       DIVERGED, REVERSED, DRIFTED,  UNMATCHED
 *
 * Both DRIFTED and REVERSED are live on this tree, so the worklist the UI
 * presents as "start at the top" was ordering real work wrongly — not a future
 * risk, a present one. Filed as N89 and fixed here.
 */
export const STATE_SEVERITY = Object.freeze([
  "DIVERGED",
  "REVERSED",
  "DRIFTED",
  "UNMATCHED",
...
/** Rank for sorting. An unknown state sorts LAST rather than first — a state
 *  nobody has ranked is not an emergency, and returning 0 would make it one. */
export function severityRank(state) {
  const i = STATE_SEVERITY.indexOf(state);
...
export function fixOrder(graph, opts = {}) {
  ...
  // Layer first — that is the whole point of the worklist, roots before
  // leaves. Then SEVERITY, not name: see STATE_SEVERITY above for what the two
  // orderings disagree about and why it mattered. Path last, only for
  // stability: a worklist that reshuffles between runs is one nobody can work
  // through.
  items.sort(
    (a, b) =>
      a.layer - b.layer ||
