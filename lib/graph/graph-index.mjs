...
function defaultDecisionsFiles(workspaces) {
  ...
  for (const w of workspaces) {
    // The v3 layout, and where every real ledger has lived since 2026-08-21:
    // <workspace>/propagation/state/<project>/DECISIONS.md. Added 2026-08-29.
    // Without this the two paths below resolve, at every workspace root, to the
    // 15-line "# DECISIONS.md — moved … Do not edit this file" pointer stubs the
    // move left behind — 0 `##` headings each. The decision tier was therefore
    // built from nothing and reported clean. Canonical layout:
    // docs/REFERENCE.md §"Propagation layout".
    const stateDir = path.join(w.root, "propagation", "state");
    if (existsSync(stateDir)) {
      let projects = [];
      try { projects = readdirSync(stateDir, { withFileTypes: true }); } catch { projects = []; }
      for (const d of projects) {
        if (!d.isDirectory()) continue;
        const p = path.join(stateDir, d.name, "DECISIONS.md");
        if (existsSync(p)) out.push(p);
      }
    }
    // Legacy/pre-move locations. Kept: a repo that has not adopted the v3 layout
    // still keeps its ledger at one of these, and a pointer stub costs one parse
    // returning zero entries. Dedup below, because a workspace can hit both.
...
export async function buildModel(workspaces, opts = {}) {
  ...
  return {
    ...
    stats: {
      ...
      // COUNT THE KIND, never "everything that is not a file". This was
      // `nodes.length - fileKeys.length`, which silently counts the PROJECT tier
      // too: 762 file + 45 project + 0 decision reported `decisions: 45`, exactly
      // matching `projects: 45` below. Line 147 promises this stat makes "zero
      // decisions visibly a finding rather than a silent pass" — computed that way
      // it did the opposite, and hid a decision tier that was empty for as long as
      // the project tier has existed. rule:enforcement-watches-itself.
