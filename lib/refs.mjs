...
import { parsePorcelain } from "./worktrees.mjs";
import { discoverWorkspacesSync, BACKUP_DIR_PREFIXES, isSkippedDir } from "./discovery.mjs";
import { findRepoRoot, getGitContext } from "./git-context.mjs";
...
export async function enumerateRefs(repoRoot) {
  ...
  try {
    ...
    for (const line of stdout.split("\n")) {
      ...
      if (tab === -1) continue; // malformed line, skip rather than misparse
      const name = trimmed.slice(0, tab);
      const sha = trimmed.slice(tab + 1);
