...
import { STATE_DIR, SKILL_DIR } from "./config.mjs";
...
export const EXPECTATIONS = [
  ...
  },
  {
    ...
    describe: "docs.supersession_prose_only <= 107 (ratchet: must not grow)",
    // A ratchet, not an equality. 107 docs claim supersession in prose today; failing on
    // all of them would print 107 findings on day one, and noise is a hiding place (G23).
    // Failing only on GROWTH makes the check honest and still able to fail.
    assert: (m) => (m["docs.supersession_prose_only"] ?? 0) <= 107,
    basis:
      "Measured 2026-08-15: 107 docs say `supersede` in prose with no `supersedes:` " +
    ...
    detail: (m) => {
      ...
      return n <= 107 ? `${n} prose-only (baseline 107)` : `${n} — GREW past the 107 baseline`;
    },
  },
  {
    ...
    detail: (m, ctx) =>
      `SEARCH_ROOTS=[${(ctx?.searchRoots || []).join(", ")}] — zero workspaces found; every ` +
      `per-workspace check below silently did not run`,
  },
  {
