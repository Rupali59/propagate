...
export const SEARCH_ROOTS = [
  ...
  /*
   * Explicit repo-level roots.
   *
   * `discoverWorkspacesSync` stops descending as soon as it finds a marker, so
   * the `.propagates.yml` at the GitHub hub root makes the hub the one and only
   * discovered workspace — every nested marker is swept up by it and every
   * project's drift lands in the hub's untracked `.propagation/ledger.jsonl`.
   *
   * Listing a repo here makes discovery walk it as its own root, so it becomes a
   * real workspace with its own ledger, versioned alongside the code that caused
   * the drift. `findAllSidecarsRecursive` applies nearest-ancestor scoping so the
   * hub sweep hands these off rather than double-processing them.
   *
   * This is opt-in per repo on purpose: promoting every nested marker at once
   * would relocate the ledgers of workspaces we have not audited.
   */
  path.join(HOME, "Documents", "GitHub", "Keerti", "Keerti-portfolio"),
];
