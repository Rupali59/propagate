...
export function brokenPathCitations(docPath, roots, readFile = (p) => readFileSync(p, "utf8")) {
  let raw;
  try {
  ...
  const broken = [];
  for (const m of raw.matchAll(CITED_PATH)) {
    const cited = m[1];
    if (cited.startsWith("http") || cited.startsWith("~")) continue;
    // A directory reference ends in "/"; anything else must look like a file.
    if (!cited.endsWith("/") && !FILE_EXT.test(cited)) continue;
