...
import { resolveTag, tagTableStatus } from "./tags.mjs";
...
import { STATE_DIR, HUB_ROOT } from "../core/config.mjs";
import { HOME } from "../core/paths.mjs";

// The CONFIGURED hub, so `PROPAGATE_HUB_ROOT` can point a test at a temp tree.
// It could not before, and the cost is recorded in tags.mjs's own note: a
// `sync --apply` test wrote a fixture line into a real register in another repo,
// because this path was a HOME-derived guess that no scoping reached. `null`
// when unconfigured (G24); the `no-tag-mappings` guard in `syncReminders` then
// refuses the run before any register path is built.
const GITHUB_ROOT = HUB_ROOT ?? path.join(HOME, "Documents/GitHub");
...
export async function syncReminders({
  ...
  tagTableStatusFn = resolveTagFn === resolveTag ? tagTableStatus : null,
} = {}) {
  // tags.mjs rule 3. The tag table is DERIVED from `.sidecar.yml` files now, so
  // it can come back empty for reasons that have nothing to do with the tags on
  // the reminders: an unconfigured hub, an unparseable sidecar, or a tree that
  // has not adopted `reminder_tags:` yet. Every one of those would otherwise
  // disposition EVERY row as `held-unknown-tag`, which tells the user they
  // used bad tags when the truth is that nothing could be looked up.
  //
  // So refuse the run instead. `ok: false` is the same shape a revoked TCC
  // permission produces, and the CONFLICTS panel already renders it as "could
  // not read" with no counts rather than as "nothing held".
  //
  // Skipped entirely when a caller injects `resolveTagFn` — an injected
  // resolver has no sidecars behind it, and gating on the real tree's state
  // would make every hermetic test depend on this machine.
  if (tagTableStatusFn) {
    const tags = tagTableStatusFn();
    if (!tags.ok) {
      return {
        ok: false,
        applied: false,
        code: "no-tag-mappings",
        reason: "could not build the tag -> project table",
        reasonDetail: tags.reason,
      };
    }
  }
  ...
  // FOUND 2026-09-25, live. This read is the SECOND osascript call of the run
  // (reconcile made the first), and the Reminders app times out often enough
  // that it matters -- one of this session's own runs came back
  // `timeout after 15000ms`. `raw.ok ? raw.items : []` then yielded an EMPTY
  // map, every lookup below missed, and `rawItem?.title ?? "(untitled)"`
  // planned three inserts reading `### PR-002 · (untitled)`.
  //
  // A failed read must never degrade into a plausible value. Refuse the run:
  // the first read succeeded, so the rows are real, but their TITLES are not
  // available and a title is the entire content of an inserted line.
  if (!raw.ok) {
    return {
      ok: false,
      applied: false,
      code: "titles-unavailable",
      reason: "the reminders list was read once but not twice",
      reasonDetail:
        `reconcile read the list, then the re-read for titles failed (${raw.reason}` +
        `${raw.reasonDetail ? `: ${raw.reasonDetail}` : ""}). An inserted line IS its title, ` +
        "so nothing is planned rather than inserting placeholders.",
    };
  }
  const rawById = new Map(raw.items.map((it) => [it.id, it]));
  ...
  for (const row of recon.rows) {
    ...
    // Belt and braces for the same defect one row down. With the `raw.ok` guard
    // above, a missing entry here means the two reads disagreed about WHICH
    // items exist -- a reminder deleted between them. That is a real fact and
    // gets reported, never papered over with a placeholder title.
    if (!rawItem || typeof rawItem.title !== "string" || !rawItem.title.trim()) {
      rows.push({
        ...row,
        disposition: "held-unrecognized-shape",
        reason: rawItem
          ? `reminder has no usable title (${JSON.stringify(rawItem.title)}) — an inserted line is its title`
          : "reminder vanished between the two reads of the list — not inserting a placeholder",
        registerPath: target.registerPath,
      });
      continue;
    }
    const insertText = `### ${row.prId} · ${rawItem.title}`;
