 ...
 *
 * FIXTURE INJECTION (`fixturePath`, or `PROPAGATE_REMINDERS_FIXTURE` read by
 * the command layer). A JSON file holding `{status, stdout, stderr}` — the
 * exact shape `osascript.mjs#readRaw` returns — so a CLI-level test can drive
 * every branch of `classify()` without a real subprocess or the real
 ...
 * seams rather than test-only hacks.
 */
...
export async function readReminders(opts = {}) {
  const { listName = DEFAULT_LIST, exec, fixturePath, timeoutMs } = opts;
