...
import { SEARCH_ROOTS, HUB_ROOT } from "../lib/core/config.mjs";
...
/**
 * Where `ECOSYSTEM.md` is written: the DECLARED HUB, falling back to the first
 * search root, and `null` when there is neither.
 *
 * ISSUES N101. This used to be `SEARCH_ROOTS[0]` alone, and `searchRoots` is a
 * DISCOVERY setting — the list of places to walk looking for `.propagates.yml`,
 * ordered for the walk and not to name the tree. When `Rupali/Experiments` was
 * added to that list for an unrelated reason it sorted first, and the artifact
 * silently relocated to `Rupali/Experiments/ECOSYSTEM.md` — a file that has
 * never existed — while the real one at the hub root, generated by this command
 * and cited by four documents, quietly stopped being regenerable.
 *
 * Nothing moved the file. A config key was added and a derived path followed
 * it. That is G24 through the other door, and it went unnoticed because this
 * function had NO TEST AT ALL.
 *
 * `ECOSYSTEM.md` rolls up the WHOLE tree and `hubRoot` is the declared name for
 * that tree, so writing it inside one nested search root is incoherent whatever
 * the ordering — a preference, not a sort.
 *
 * `null` is a could-not-run, never a crash: there is nowhere to derive relative
 * to and nowhere to write, and that is a fact about the install rather than a
 * defect in this command. `HUB_ROOT_DIAGNOSTIC` already names the fix.
 *
 * Parameters are injectable so this is testable without a subprocess; the
 * defaults are the module-level constants every caller relies on.
 */
export function artifactPath({ hubRoot = HUB_ROOT, roots = SEARCH_ROOTS } = {}) {
  const base = hubRoot || roots[0];
  return base ? path.join(base, ARTIFACT_NAME) : null;
}
