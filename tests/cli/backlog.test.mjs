...
// ─────────────────────────────────────────────────────────────────────────
// N56 — a pointer stub reaching the state-live-sections reader
// ─────────────────────────────────────────────────────────────────────────

test("N56: backlog() classifies a STATE.md pointer stub as pointer-stub, never `0 open`", () => {
  // The defect: `isPointerStubText()` existed, was correct about every one of
  // them, and was consulted by the checkbox/id-keyed paths only. The
  // state-live-sections reader was added later and never made the call, so 23
  // signposts reported `state-live-sections, 0 open` — indistinguishable from a
  // project that genuinely has no open work, and `0 open` is the reading that
  // makes work disappear (rule:discernment-checks §2 and §6).
  //
  // Both files are created so the assertion is about DISCRIMINATION, not about
  // a reader that calls everything a stub — the failure mode that would hide
  // real backlog and be worse than the bug.
  const root = tmp("n56-");
  try {
    mkdirSync(path.join(root, "ws"), { recursive: true });
    mkdirSync(path.join(root, "ws", "propagation", "state", "workspace"), { recursive: true });

    writeFileSync(
      path.join(root, "ws", "STATE.md"),
      [
        "# STATE.md — moved",
        "",
        "This is a **pointer stub**, not the state. The file now lives at",
        "`propagation/state/workspace/STATE.md`, with `.sidecar.yml` beside it.",
        "",
        "Do not edit this file. Edit the target.",
      ].join("\n"),
    );
    writeFileSync(
      path.join(root, "ws", "propagation", "state", "workspace", "STATE.md"),
      ["# ws — State", "", "## Now", "", "- a real open item", "- a second real open item", ""].join("\n"),
    );

    const r = backlog({ searchRoots: [root] });
    const byName = (suffix) => r.stateFiles.find((f) => f.file.endsWith(suffix));

    const stub = byName(path.join("ws", "STATE.md"));
    assert.ok(stub, "the stub must still be REPORTED — suppressing it is a different bug");
    assert.equal(stub.format, "pointer-stub", "must not read as state-live-sections");
    assert.equal(stub.items.length, 0);
    assert.match(stub.detail, /propagation\/state\/workspace\/STATE\.md/, "it must name where the state went");

    const real = byName(path.join("state", "workspace", "STATE.md"));
    assert.ok(real, "the real file must be discovered");
    assert.equal(real.format, "state-live-sections");
    assert.equal(real.items.length, 2, "a real STATE.md beside a stub must still yield its items");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test("pointerStubTarget reads all five phrasings the tree actually uses", () => {
  // Measured 2026-08-28: 57 real pointer stubs (classified by isPointerStubText,
  // NOT by grepping for the words "pointer stub" — that over-counts real files
  // that merely mention stubs, and is how this was first mis-measured). The
  // single `now lives at \`path\`` pattern found 47 and missed 10, every one of
  // which named its target in different words. "(target not named)" was printed
  // about files whose third line IS the path — the N56 shape one level down.
  //
  // Each case below is copied from a live stub, named in the comment. A pattern
  // nobody can point at a real file is a pattern that will drift.
  const cases = [
    ["Motherboard/STATE.md (blockquoted table row)",
      "> | target | `propagation/state/workspace/STATE.md` |\n",
      "propagation/state/workspace/STATE.md"],
    ["the hub's STATE.md (markdown link)",
      "**This file now lives at [`propagation/state/workspace/STATE.md`](propagation/state/workspace/STATE.md).**",
      "propagation/state/workspace/STATE.md"],
    ["Tathya/STATE.md (no 'now', newline before the path)",
      "This is a **pointer stub**, not the state. The file lives at\n`propagation/state/workspace/STATE.md`, with `.sidecar.yml` beside it.",
      "propagation/state/workspace/STATE.md"],
    ["Tathya/WorkTracker/STATE.md ('is at', not 'lives at')",
      "This is a **pointer stub**, not the content. The file is at\n`propagation/state/WorkTracker/STATE.md` in the `Tathya` workspace.",
      "propagation/state/WorkTracker/STATE.md"],
    ["VipinKaushik/TODOS.md (bold-wrapped, sibling target)",
      "This file moved to **`STATE.md`** on 2026-06-08 as part of the convention.",
      "STATE.md"],
  ];
  for (const [name, text, expected] of cases) {
    const r = parseTodoLikeFile("# TODOS.md — moved\n\n" + text + "\n\nDo not edit this file.\n", "/tmp/TODOS.md");
    assert.equal(r.format, "pointer-stub", `${name}: must classify as a pointer stub`);
    assert.equal(r.stubReason, `pointer stub — state lives at ${expected}`, `${name}: wrong target`);
  }
});

test("pointerStubTarget returns null rather than naming something that is not a path", () => {
  // Naming the WRONG target is worse than naming none, because a reader follows
  // it. The `.md` guard is what stops a loose pattern like `is at` from
  // capturing prose. Absence here is honest; a confident wrong path is not.
  const r = parseTodoLikeFile(
    "# TODOS.md — moved\n\nThis is a **pointer stub**. The risk is at `HEAD` on the release branch.\n\nDo not edit.\n",
    "/tmp/TODOS.md",
  );
  assert.equal(r.format, "pointer-stub");
  assert.equal(r.stubReason, "pointer stub — state lives elsewhere (target not named)");
});

test("an explicit `| target |` row beats prose in the same stub", () => {
  // A file carrying both must not let the prose win: the table row is a
  // declaration, the sentence is description.
  const r = parseTodoLikeFile(
    "# TODOS.md — moved\n\nThis is a **pointer stub**. It moved to `docs/OLD.md` once.\n\n| target | `propagation/state/workspace/TODOS.md` |\n",
    "/tmp/TODOS.md",
  );
