...
import { notifyKey, selectToNotify, formatNotification } from "../lib/monitor.mjs";
import { isActionable } from "../lib/graph.mjs";
...
test("readNotified survives a corrupt line and fails OPEN", async (t) => {
  ...
  const { readNotified } = await import("../lib/monitor.mjs");
...
test("every run logs, including one that notifies nothing", async (t) => {
  ...
  const { logRun } = await import("../lib/monitor.mjs");
