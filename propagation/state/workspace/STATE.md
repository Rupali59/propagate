...
`artifactPath()` returned `SEARCH_ROOTS[0]`, and `searchRoots` is a **discovery** setting — the
list of directories to walk for `.propagates.yml`, ordered for the walk, not to name the tree.
`Rupali/Experiments` sits first in it, so the artifact resolved to
`Rupali/Experiments/ECOSYSTEM.md` — a file that has never existed — while the real one at the
hub root, generated by this command and cited by four documents, sat untouched since 2026-09-01.

**Nothing moved the file.** A config key was added for an unrelated reason and a derived path
followed it. The symptom was the giveaway and I had already seen it without reading it properly:
`rollup --check` exited `2 could-not-run` saying the artifact "does not exist yet" — a command
reporting it cannot find something it would itself create, while the thing it was actually
maintaining went stale in silence.

**`artifactPath()` had no test at all.** Not a weak one. The function deciding where the tool's
only generated artifact lands was unasserted, so the path could move without a single red.

Fixed to prefer the declared `hubRoot`, fall back to the first root, and return `null` with
neither. The negative control is the assertion that matters — **reordering `searchRoots` must
not move the file** — and under the old code it would have passed, both calls agreeing and both
wrong.

`rollup --check` went `2` → `1` (stale, correctly) → `0` (current). `ECOSYSTEM.md` regenerated
at the hub root, 734 → 959 lines, with Khushboo and Rishabh now reading **"NON-CONFORMANT,
present but EMPTY"** beside Sindhu's **"Missing 4 of 5"** — N82's two vocabularies, visible in
the artifact.

Suite: **2219 + 94, 0 failures.**
