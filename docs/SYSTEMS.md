...
| `skill-registry` | Node lib + CLI modes in `propagate`, reaper rides the existing digest | active | — | `lib/skills-scan.mjs`, `lib/skills-lifecycle.mjs`, `lib/skills-create.mjs`; `~/Documents/GitHub/skills-marketplace/`; `SKILLS_LIFECYCLE.jsonl`; `skill` table in `index.db`; kill switch `~/.claude/skills-registry.off` | `node cli.mjs skills` runs and classifies every skill by provenance (gstack-relink / handmade / npx-skills) and reports both lifecycle tiers; `node cli.mjs skills-reap` names only quarantined skills unused for >14d. **The probe is that it classifies, not a count** — the total was 169 when this was planned, 92 when the row was written and 61 on 2026-08-17, because skills are consolidated continuously. Naming the number here is how a state file rots (rule:state-and-decisions). | 2026-08-11 | 2026-08-16 — `public-site-audit` completed the full quarantine -> promoted path without hand-holding (`SKILLS_LIFECYCLE.jsonl`); it now lives at `skills-marketplace/tathya/skills/public-site-audit`. Two skills remain in quarantine and both register live as `quarantine:changelog-entry` and `quarantine:env-pull-check`. Nothing has been reaped yet, which is expected — neither quarantined skill is both unused and older than 14d. | n/a |
 ...
 | `propagate-monitor` | launchd agent (Node). `WatchPaths` on discovered workspace roots, `ThrottleInterval` 300, `StartInterval` 1800 (48 runs/day, against the retired watcher's 1,440) | **ARMED 2026-08-17** — loaded via `launchctl bootstrap`; first production run 5088ms over 724 edges, 8 notified, exit 0 | — | `lib/monitor.mjs`, `cli.mjs` `monitor` mode, `lib/plist.mjs` `writeMonitorPlist`; state `~/.propagate/notified.jsonl` + `~/.propagate/monitor.log`; label `com.tathya.propagate.monitor`, deliberately NOT the retired watcher's | `tail ~/.propagate/monitor.log` distinguishes THREE states, which is what makes it a probe: **never ran** (no file), **ran and found nothing** (`notified=0`), **ran and told you** (`notified>0`). Whether it is armed is a separate question, and `monitor --install` prints the exact arm/disarm commands rather than this row carrying them — a row naming an uninstalled label beside a shell command is a line someone copies and is refused by tests/skill-doc.test.mjs. A monitor that has silently died is indistinguishable from a quiet week without that log — the same absence that let a zombie LaunchAgent write 37 MB of stderr over six weeks. | 2026-08-17 | **BLANK — fill only when a notification it sent led to an edge being resolved sooner than the daily digest would have.** Firing is not helping. This is the component most likely to become the thing everyone swipes away, and the v1 watcher earned its retirement on exactly that distinction (4,420 runs, 99.2% no-ops). | n/a | 
...
| `inventory-probe` | CLI + digest section | **active-unadopted** | — | `lib/inventory.mjs`; `cli.mjs inventory`; INVENTORY block in `digest.mjs` | `node ~/.claude/skills/propagate/cli.mjs inventory \| head -3` | 2026-08-14 | — *not earned* | n/a |
| `skills-marketplace` | directory tree + lifecycle CLI | **installed-never-invoked** | — | `~/Documents/GitHub/skills-marketplace/{tathya,quarantine}/skills`; `lib/skills-lifecycle.mjs`; `SKILLS_LIFECYCLE.jsonl` | `ls ~/Documents/GitHub/skills-marketplace/tathya/skills \| wc -l` → 0. **Manages a tree the 92 installed skills do not live in** — its 3 pre-2026-08-14 log events are two test skills | 2026-08-14 | — *not earned* | n/a |
