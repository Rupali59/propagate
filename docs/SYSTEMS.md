...
|---|---|---|---|---|---|---|---|---|
| `propagate` | launchd agent (Node, StartInterval 60) | **retired 2026-08-14** | superseded_by: `reconcile` (on demand) + `check` (pre-push) + the daily digest's DRIFT/INBOUND sections — see `docs/DECISIONS.md` 2026-08-14 for the evidence (4,420 runs / 4,384 no-ops, 99.2%) and the full tradeoff (sub-daily proactive notification is genuinely lost). None of these three replacements are themselves launchd agents — they're on-demand/CLI, so this row has no direct successor row in this table; that absence is the point, not an omission. | `~/Library/LaunchAgents/com.tathya.propagate.watcher.plist`; `~/.claude/skills/propagate/heartbeat`; `watcher.mjs` (kept on disk, header records the retirement, refuses to run directly without `PROPAGATE_ALLOW_RETIRED_WATCHER=1`) | Retired: heartbeat age is no longer a health signal (`cli.mjs doctor` and `digest.mjs` both stopped treating it as one 2026-08-14 — see the DECISIONS.md entry). The replacement's liveness probe is now: `node cli.mjs doctor` → "v2 replacement" section → "event store readable" + "reconcile completes" both green. | 2026-08-14 (code+docs side of retirement verified: `cli.mjs doctor` no longer reports watcher checks as failures, reports replacement health instead; 470/470 tests pass) | 2026-08-10 (watcher itself, historical — retired 2026-08-14, see status column) | **partial** — code path retired, `doctor`/`digest.mjs` updated, `SKILL.md`/`docs/REFERENCE.md`/`docs/ISSUES.md` updated, this row updated, all in this change. The launchd unload/disable itself is a **separate, later operational step** done outside this change (task constraint: no `launchctl` commands run here) — do not mark this `done` until that step is confirmed and this row is updated again to say so. `.propagates.yml` removal / hub repo map update are not applicable (the watcher was never itself a client workspace). |
...
- 2026-08-14: `propagate`'s row status moved `active` → `retired`, per
  ...
  `launchctl list | grep com.tathya.propagate.watcher` returns nothing first.
